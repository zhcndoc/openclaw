---
title: "展示"
description: "社区中的真实 OpenClaw 项目"
summary: "由 OpenClaw 驱动的社区构建项目和整合"
read_when:
...
来自社区的真实项目。看看大家用 OpenClaw 构建了什么。

<Info>
**想要展示你的项目？** 在 [Discord 的 #showcase 频道](https://discord.gg/clawd) 分享你的项目，或在 [X 上标记 @openclaw](https://x.com/openclaw)。
</Info>

## 🎥 OpenClaw 实战

VelvetShark 的完整设置演示（28分钟）。
...
>
  <iframe
    ...
    title="OpenClaw：Siri 应该拥有的自托管 AI（完整设置）"
...
<Card title="PR 审核 → Telegram 反馈" icon="code-pull-request" href="https://x.com/i/status/2010878524543131691">
  **@bangnokia** • `review` `github` `telegram`

OpenCode 完成更改 → 提交 PR → OpenClaw 审核差异并在 Telegram 回复，提供“细节建议”和明确的合并意见（包括优先修复的关键问题）。
...
向“Robby” (@openclaw) 请求一个本地的酒窖技能。它请求一个示例 CSV 导出和存储位置，然后快速构建/测试技能（示例中有962瓶酒）。
...
每周餐食计划 → 常购清单 → 预约送货时间 → 确认订单。无 API，纯浏览器控制。
...
热键捕捉屏幕区域 → Gemini 视觉识别 → 即刻生成 Markdown 并复制到剪贴板。
...
桌面应用，管理 Agents、Claude、Codex 和 OpenClaw 之间的技能与指令。
...
通过 Homebrew 安装的工具来列举/检查/监控本地 OpenAI Codex 会话（CLI + VS Code 集成）。
...
上传至 Cloudflare R2/S3 并生成安全的预签名下载链接。最适合远程 OpenClaw 实例。
</Card>
...
通过 Opus 4.5 协调器在一个网关下管理 14+ 代理，分别委派给 Codex 工作者。详尽的[技术说明](https://github.com/adam91holt/orchestrated-ai-articles)涵盖成员名单、模型选择、沙盒、Webhook、心跳与委派流程。[Clawdspace](https://github.com/adam91holt/clawdspace)用于代理沙盒。[博客文章](https://adams-ai-journey.ghost.io/2026-the-year-of-the-orchestrator/)。
</Card>

<Card title="Linear CLI" icon="terminal" href="https://github.com/Finesssee/linear-cli">
  **@NessZerra** • `devtools` `linear` `cli` `issues`

适用于 Linear 的 CLI，集成代理工作流（Claude Code、OpenClaw）。可在终端管理问题、项目和工作流。首个外部 PR 已合并！
</Card>

<Card title="Beeper CLI" icon="message" href="https://github.com/blqke/beepcli">
  **@jules** • `messaging` `beeper` `cli` `automation`

通过 Beeper Desktop 读取、发送和归档消息。使用 Beeper 本地 MCP API，助力代理统一管理 iMessage、WhatsApp 等所有聊天。
</Card>
...
屋顶摄像头触发：只要天空漂亮，就让 OpenClaw 拍一张照片——它设计了技能并完成了拍摄。

  <img src="/assets/showcase/roof-camera-sky.jpg" alt="由 OpenClaw 捕捉的屋顶摄像头天空快照" />
</Card>

<Card title="视觉晨间简报画面" icon="robot" href="https://x.com/buddyhadry/status/2010005331925954739">
  **@buddyhadry** • `automation` `briefing` `images` `telegram`

定时提示每日早晨生成一张“场景”图像（天气、任务、日期、喜欢的帖子/名言），通过 OpenClaw 角色实现。
</Card>

<Card title="Padel 球场预订" icon="calendar-check" href="https://github.com/joshp123/padel-cli">
  **@joshp123** • `automation` `booking` `cli`
  
  Playtomic 可用性检测与预订 CLI。再也不会错过空闲球场。
  
  <img src="/assets/showcase/padel-screenshot.jpg" alt="padel-cli 截图" />
</Card>

<Card title="会计材料收集" icon="file-invoice-dollar">
  **社区** • `automation` `email` `pdf`
  
  收集邮件中的 PDF，准备给税务顾问用。月度会计流程自动化。
</Card>

<Card title="懒人开发模式" icon="couch" href="https://davekiss.com">
  **@davekiss** • `telegram` `website` `migration` `astro`

边看 Netflix 边用 Telegram 重建个人网站——将 Notion 内容迁移到 Astro，迁移18篇文章，DNS 切换到 Cloudflare。一次笔记本都没开。
</Card>

<Card title="求职代理" icon="briefcase">
  **@attol8** • `automation` `api` `skill`

搜索职位，按简历关键词匹配，返回相关机会及链接。使用 JSearch API，30 分钟内完成。
</Card>

<Card title="Jira 技能生成器" icon="diagram-project" href="https://x.com/jdrhyne/status/2008336434827002232">
...
通过浏览器自动登录 TradingView，截图图表并按需进行技术分析。无 API，仅浏览器控制。
</Card>

<Card title="Slack 自动支持" icon="slack">
  **@henrymascot** • `slack` `automation` `support`

监控公司 Slack 频道，提供帮助回复并将通知转发 Telegram。无人指令下自主修复了线上应用的生产缺陷。
</Card>
...
<Card title="xuezh 中文学习" icon="language" href="https://github.com/joshp123/xuezh">
  **@joshp123** • `learning` `voice` `skill`
  ...
  基于 OpenClaw 的中文学习引擎，提供发音反馈和学习流程。
...
<Card title="Home Assistant 插件" icon="home" href="https://github.com/ngutman/openclaw-ha-addon">
  **@ngutman** • `homeassistant` `docker` `raspberry-pi`
  ...
  在 Home Assistant OS 上运行的 OpenClaw 网关，支持 SSH 隧道及持久状态。
</Card>
...
<Card title="Nix 打包" icon="snowflake" href="https://github.com/openclaw/nix-openclaw">
  **@openclaw** • `nix` `packaging` `deployment`
  ...
  含电池包的 nix 化 OpenClaw 配置，保证可复现部署。
</Card>
...
<Card title="GoHome 自动化" icon="house-signal" href="https://github.com/joshp123/gohome">
  **@joshp123** • `home` `nix` `grafana`
  ...
  Nix 原生家庭自动化，OpenClaw 作为界面，附带美观 Grafana 仪表盘。
  
  <img src="/assets/showcase/gohome-grafana.png" alt="GoHome Grafana 仪表盘" />
</Card>

<Card title="石头机器人吸尘器" icon="robot" href="https://github.com/joshp123/gohome/tree/main/plugins/roborock">
  **@joshp123** • `vacuum` `iot` `plugin`
  
  通过自然对话控制你的石头机器人吸尘器。
  
  <img src="/assets/showcase/roborock-screenshot.jpg" alt="石头机器人状态" />
</Card>
...
<Steps>
  <Step title="分享它">
    在 [Discord 的 #showcase 频道](https://discord.gg/clawd) 或 [在 X 上 Tweet @openclaw](https://x.com/openclaw)
  </Step>
  <Step title="包含细节">
    告诉我们项目功能，提供仓库/演示链接，如有截图请分享
  </Step>
  <Step title="获得展示">
    我们会将优秀项目添加到此页面
  </Step>
