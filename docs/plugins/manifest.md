...
| 字段                                 | 必填     | 类型                         | 含义                                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------------------ | -------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `id`                                 | 是       | `string`                     | 规范插件 ID。此 ID 用于 `plugins.entries.<id>`。例外情况：如果某个软件包的 `package.json` 声明了多个插件条目，则每个条目都会注册为 `<id>/<entry-basename>`（例如 `pack/one`），该条目作用域内的 ID 就是该条目的 `plugins.entries` 键。条目基本名称在软件包内必须唯一；如果基本名称冲突，将在发现阶段拒绝。 |
| `configSchema`                       | 是       | `object`                     | 此插件配置的内联 JSON Schema。                                                                                                                                                                                                                                                                                                                                                     |
| `requiresPlugins`                    | 否       | `string[]`                   | 此插件要生效必须同时安装的插件 ID。如果缺少任何必需插件，发现过程仍会保留该插件的可加载状态，但会发出警告。                                                                                                                                                                                                                                                                                                     |
| `enabledByDefault`                   | 否       | `true`                       | 将捆绑插件标记为默认启用。省略此字段，或将其设置为任何非 `true` 的值，即可使插件默认禁用。                                                                                                                                                                                                                                                                     |
...
| `providerCatalogEntry`               | 否       | `string`                     | 轻量级提供商目录模块路径，相对于插件根目录，用于提供清单作用域内的提供商目录元数据；无需激活完整插件运行时即可加载这些元数据。                                                                                                                                                                                                              |
| `modelSupport`                       | 否       | `object`                     | 清单拥有的简写模型系列元数据，用于在运行时之前自动加载插件。                                                                                                                                                                                                                                                                                                      |
| `modelCatalog`                       | 否       | `object`                     | 插件所拥有提供商的声明式模型目录元数据。这是未来实现只读列表、引导流程、模型选择器、别名和抑制功能的控制平面契约，且无需加载插件运行时。                                                                                                                                                                      |
| `modelPricing`                       | 否       | `object`                     | 由提供商拥有的托管定价发布策略。可用它将本地/自托管提供商排除在已发布定价之外，或将提供商引用映射到 OpenRouter/LiteLLM 目录 ID，而无需在核心代码中硬编码提供商 ID。                                                                                                                                                                             |
| `modelIdNormalization`               | 否       | `object`                     | 由提供商负责的模型 ID 别名/前缀清理，必须在加载提供商运行时之前执行。                                                                                                                                                                                                                                                                                                        |
| `providerEndpoints`                  | 否       | `object[]`                   | 由清单拥有的端点主机/baseUrl 元数据，用于在加载提供商运行时之前对提供商路由进行分类。                                                                                                                                                                                                                                                                         |
| `providerRequest`                    | 否       | `object`                     | 由通用请求策略使用的、成本低廉的提供商系列和请求兼容性元数据，用于在加载提供商运行时之前执行判断。                                                                                                                                                                                                                                                                           |
| `secretProviderIntegrations`         | 否       | `Record<string, object>`     | 声明式 SecretRef exec 提供商预设，使设置或安装界面能够提供这些预设，而无需在核心代码中硬编码特定提供商的集成。                                                                                                                                                                                                                                                  |
| `cliBackends`                        | 否       | `string[]`                   | 此插件拥有的 CLI 推理后端 ID。用于根据显式配置引用在启动时自动激活。                                                                                                                                                                                                                                                                                      |
| `syntheticAuthRefs`                  | 否       | `string[]`                   | 提供商或 CLI 后端引用。在冷模型发现期间、加载运行时之前，应探测这些引用对应的插件自有合成身份验证钩子。                                                                                                                                                                                                                                                           |
| `nonSecretAuthMarkers`               | 否       | `string[]`                   | 由捆绑插件拥有的占位 API 密钥值，用于表示非机密的本地、OAuth 或环境凭据状态。                                                                                                                                                                                                                                                                             |
| `commandAliases`                     | 否       | `object[]`                   | 此插件拥有的命令名称。在加载运行时之前，这些名称应生成了解插件的配置和 CLI 诊断信息。                                                                                                                                                                                                                                                                             |
| `providerUsageAuthEnvVars`           | 否       | `Record<string, string[]>`   | 仅用于使用量/计费的提供商凭据。OpenClaw 使用这些名称进行使用量发现和机密清理，但绝不将其用于推理身份验证。                                                                                                                                                                                                                                                        |
| `providerAuthAliases`                | 否       | `Record<string, string>`     | 应在身份验证查找中复用另一个提供商 ID 的提供商 ID，例如共享基础提供商 API 密钥和身份验证配置的编码提供商。                                                                                                                                                                                                                                       |
| `providerAuthChoices`                | 否       | `object[]`                   | 用于引导选择器、首选提供商解析和简单 CLI 标志接线的低成本身份验证选择元数据。                                                                                                                                                                                                                                                                                    |
| `activation`                         | 否       | `object`                     | 用于启动、提供商、命令、频道、路由和能力触发式加载的低成本激活规划器元数据。仅限元数据；插件运行时仍负责实际行为。                                                                                                                                                                                                                    |
| `setup`                              | 否       | `object`                     | 低成本设置/引导描述符，发现和设置界面无需加载插件运行时即可检查这些描述符。                                                                                                                                                                                                                                                                                 |
| `qaRunners`                          | 否       | `object[]`                   | 共享的 `openclaw qa` 主机在加载插件运行时之前使用的低成本 QA 运行器描述符。                                                                                                                                                                                                                                                                                                   |
| `dashboard`                          | 否       | `object`                     | 控制面板小组件数据绑定和操作动词。每个条目都会根据此插件注册且具有所需读或写作用域的 Gateway 方法进行验证。请参阅[控制面板参考](#dashboard-reference)。                                                                                                                                                                              |
| `mcpServers`                         | 否       | `Record<string, object>`     | 此插件启用期间提供的静态 MCP 服务器定义。相对命令参数和工作目录均相对于插件根目录解析。操作员的 `mcp.servers` 条目会覆盖或禁用同名定义。请参阅 [MCP 服务器参考](#mcp-server-reference)。                                                                                                   |
| `contracts`                          | 否       | `object`                     | 静态能力所有权快照，涵盖外部身份验证钩子、嵌入、语音、实时转录、实时语音、媒体理解、图像/视频/音乐生成、网页抓取、网页搜索、工作器提供商、文档/网页内容提取和工具所有权。                                                                                                                       |
| `configContracts`                    | 否       | `object`                     | 由清单拥有、供通用核心辅助函数使用的配置行为：危险标志检测、SecretRef 迁移目标和旧版配置路径收窄。请参阅 [configContracts 参考](#configcontracts-reference)。                                                                                                                                                                           |
...
| `imageGenerationProviderMetadata`    | 否       | `Record<string, object>`     | 为 `contracts.imageGenerationProviders` 中声明的提供商 ID 提供的低成本图像生成身份验证元数据，包括提供商自有的身份验证别名和基础 URL 守卫。                                                                                                                                                                                                                               |
| `videoGenerationProviderMetadata`    | 否       | `Record<string, object>`     | 为 `contracts.videoGenerationProviders` 中声明的提供商 ID 提供的低成本视频生成身份验证元数据，包括提供商自有的身份验证别名和基础 URL 守卫。                                                                                                                                                                                                                               |
| `musicGenerationProviderMetadata`    | 否       | `Record<string, object>`     | 为 `contracts.musicGenerationProviders` 中声明的提供商 ID 提供的低成本音乐生成身份验证元数据，包括提供商自有的身份验证别名和基础 URL 守卫。                                                                                                                                                                                                                               |
| `toolMetadata`                       | 否       | `Record<string, object>`     | 为 `contracts.tools` 中声明的插件自有工具提供的低成本可用性元数据。当工具只有在存在配置、环境变量或身份验证凭据等证据时才应加载运行时时使用。                                                                                                                                                                                                                        |
...
| `icon`                               | 否       | `string`                     | 用于市场/目录卡片的 HTTPS 图像 URL。ClawHub 接受任何有效的 `https://` URL；如果省略或无效，则回退到默认插件图标。                                                                                                                                                                                                                               |
...
当 `appGuidedDiscovery` 为 true 时，匹配的 provider auth method 必须提供
