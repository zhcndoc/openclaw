...
| `command`                                     | 托管的 Codex 二进制文件                                   | stdio 传输使用的可执行文件。留空则使用受管理的二进制文件。                                                                                                                                                                                                                                                                                                                                                             |
| `args`                                        | `["app-server", "--listen", "stdio://"]`               | stdio 传输使用的参数。                                                                                                                                                                                                                                                                                                                                                                                                     |
| `url`                                         | 未设置                                                  | WebSocket App Server URL 或 `unix://` URL。显式指定为空的 Unix 路径会选择规范的用户主目录控制套接字。                                                                                                                                                                                                                                                                                                             |
...
| `remoteWorkspaceRoot`                         | 未设置                                                  | 远程 Codex app-server 工作区根目录。OpenClaw 会将本地 cwd 映射到该根目录，并通过有输出上限、无 shell 的 `command/exec` 读取器传输权威的远程附件。路径逃逸任一工作区、符号链接、超大文件以及无界附件批次都会安全失败；上传会保留已配置的通道身份和 app-server 请求超时时间。                                            |
...
| `mode`                                        | `"yolo"`，除非本地 Codex 要求不允许 YOLO | YOLO 或由 guardian 审核的执行预设。                                                                                                                                                                                                                                                                                                                                                                                    |
| `approvalPolicy`                              | `"never"` 或允许的 guardian 审批策略       | 发送到 thread 启动、恢复和 turn 的原生 Codex 审批策略。                                                                                                                                                                                                                                                                                                                                                               |
| `sandbox`                                     | `"danger-full-access"` 或允许的 guardian 沙箱 | 发送到 thread 启动和恢复的原生 Codex 沙箱模式。启用的 OpenClaw 沙箱会将 `danger-full-access` turn 收窄为 Codex 的 `workspace-write`；turn 的网络标志遵循 OpenClaw 沙箱出口配置。                                                                                                                                                                                                                          |
...
| `serviceTier`                                 | 未设置                                                  | 仅用于原生 Codex app-server 的偏好设置。任何非空字符串都会原样传递，以保持向前兼容；文档规定的值为 `"priority"` 和 `"flex"`。`null` 会清除覆盖值，旧版 `"fast"` 会规范化为 `"priority"`。这既不是共享的 Fast 模式设置，也不是直接的嵌入式 OpenAI 设置。共享的 Fast 运行控制会使用 `priority` 或 `null` 覆盖它，或者在自动模式下为每次模型调用决定该值。 |
| `networkProxy`                                | 已禁用                                               | 为 app-server 命令选择加入 Codex 权限配置文件网络。OpenClaw 会定义选定的 `permissions.<profile>.network` 配置，并使用 `default_permissions` 选择它，而不是发送 `sandbox`。                                                                                                                                                                                                                |
...
当配置了原生 Codex 插件时，OpenClaw 会缓存一个按运行时和工作区作用域划分的 `plugin/installed` 快照。该快照涵盖从 Codex 发现的市场中安装的插件，包括已禁用所有权的插件；`plugin/read` 仅解析完全匹配的已配置插件身份。失败或不完整的已安装快照永远不会被缓存。`/codex plugins available` 会为当前 conversation 工作区查询 `plugin/list`，而 `/codex plugins install <plugin>@<marketplace>` 只有在所有者或 `operator.admin` 明确授权该插件后才会安装。现有明确配置的精选插件仍保留其自动恢复路径。模型的插件发现工具不能安装、启用或认证插件。
