...
通道入口是入站通道事件的实验性访问控制边界。接收路径请使用 `openclaw/plugin-sdk/channel-ingress-runtime`。较旧的 `openclaw/plugin-sdk/channel-ingress` 子路径仍然导出，作为面向第三方插件的已弃用兼容层。
...
不要预先计算有效允许名单、命令所有者或命令组。解析器会从原始允许名单、存储回调、路由描述符、访问组、策略和会话类型中推导这些内容。
...
- `commandAccess`：命令授权；当未运行命令闸门时为 false
- `activationAccess`：提及/激活结果

事件授权仍然可通过有序的 `ingress.graph` 和决定性的 `ingress.reasonCode` 获取；不会单独发出事件投影。
...
当插件有多个可选路由描述符时，请使用 `channelIngressRoutes(...)`；它会过滤掉已禁用的分支，同时保持路由事实通用，并按每个描述符的 `precedence` 保持有序。
...
原始发送者值和原始允许名单条目仅可作为解析器输入。它们不得出现在解析后的状态、决策、诊断、快照或兼容性事实中。请使用不透明的主体 ID、条目 ID、路由 ID 和诊断 ID。
