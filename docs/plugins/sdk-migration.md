...
<Steps>
  ...
  <Step title="将 embedded tool-result 扩展迁移到中间件">
    捆绑插件必须将仅适用于 embedded-runner 的
    ...
    已安装的插件在显式启用并在
    `contracts.agentToolResultMiddleware` 中声明每个目标运行时后，也可以注册 tool-result 中间件。未声明的已安装中间件注册会被拒绝。
...
<Accordion title="常见导入路径表">
  | 导入路径 | 用途 | 关键导出 |
  | --- | --- | --- |
  | `plugin-sdk/plugin-entry` | 标准化插件入口辅助工具 | `definePluginEntry` |
  | `plugin-sdk/core` | 通道入口定义/构建器的旧式总重导出 | `defineChannelPluginEntry`, `createChatChannelPlugin` |
  ...
  | `plugin-sdk/setup` | 共享 setup 向导辅助工具 | Setup translator, allowlist 提示, setup 状态构建器 |
  | `plugin-sdk/setup-runtime` | setup 期间运行时辅助工具 | `createSetupTranslator`, import-safe setup patch adapters, lookup-note helpers, `promptResolvedAllowFrom`, `splitSetupEntries`, delegated setup proxies |
  | `plugin-sdk/setup-adapter-runtime` | 已弃用的 setup adapter 别名 | 使用 `plugin-sdk/setup-runtime` |
  ...
  | `plugin-sdk/account-helpers` | 窄范围账号辅助工具 | 账号列表/账号动作辅助工具 |
  ...
  | `plugin-sdk/channel-reply-pipeline` | 回复前缀、打字状态和来源投递接线 | `createChannelReplyPipeline`, `resolveChannelSourceReplyDeliveryMode` |
  ...
  | `plugin-sdk/channel-config-schema` | 配置 schema 构建器 | 共享的通道配置 schema 原语和通用构建器 |
  | `plugin-sdk/bundled-channel-config-schema` | 捆绑配置 schema | 仅 OpenClaw 维护的捆绑插件；新插件必须定义插件本地 schema |
  | `plugin-sdk/channel-config-schema-legacy` | 已弃用的捆绑配置 schema | 仅兼容别名；对受维护的捆绑插件请使用 `plugin-sdk/bundled-channel-config-schema` |
  ...
  | `plugin-sdk/channel-lifecycle` | 已弃用的兼容外观 | 使用 `plugin-sdk/channel-outbound` |
  | `plugin-sdk/inbound-envelope` | 入站信封辅助工具 | 共享 route + 信封构建器辅助工具 |
  | `plugin-sdk/channel-inbound` | 入站接收辅助工具 | 上下文构建、格式化、根、运行器、已准备回复分发和分发谓词 |
  | `plugin-sdk/messaging-targets` | 已弃用的目标解析导入路径 | 通用目标解析辅助工具请使用 `plugin-sdk/channel-targets`，路由比较请使用 `plugin-sdk/channel-route`，provider 特定目标解析请使用插件拥有的 `messaging.targetResolver` / `messaging.resolveOutboundSessionRoute` |
  | `plugin-sdk/outbound-media` | 出站媒体辅助工具 | 共享出站媒体加载 |
  | `plugin-sdk/outbound-send-deps` | 已弃用的兼容外观 | 使用 `plugin-sdk/channel-outbound` |
  | `plugin-sdk/channel-outbound` | 出站消息生命周期辅助工具 | 消息适配器、收据、持久化发送辅助工具、实时预览/流式传输辅助工具、回复选项、生命周期辅助工具、出站标识和负载规划 |
  | `plugin-sdk/channel-streaming` | 已弃用的兼容外观 | 使用 `plugin-sdk/channel-outbound` |
  | `plugin-sdk/outbound-runtime` | 已弃用的兼容外观 | 使用 `plugin-sdk/channel-outbound` |
  | `plugin-sdk/thread-bindings-runtime` | 线程绑定辅助工具 | 线程绑定生命周期和适配器辅助工具 |
  | `plugin-sdk/agent-media-payload` | 旧媒体负载辅助工具 | 面向旧字段布局的 agent 媒体负载构建器 |
  ...
  | `plugin-sdk/gateway-runtime` | 网关辅助工具 | 网关客户端、事件循环就绪启动辅助工具和 channel-status 补丁辅助工具 |
  ...
  | `plugin-sdk/telegram-command-config` | Telegram 命令辅助工具 | 当捆绑的 Telegram 契约面不可用时的回退稳定 Telegram 命令验证辅助工具 |
  | `plugin-sdk/approval-runtime` | 审批提示辅助工具 | exec/plugin 审批负载、审批能力/配置文件辅助工具、原生审批路由/运行时辅助工具，以及结构化审批显示路径格式化 |
  ...
  | `plugin-sdk/approval-gateway-runtime` | 审批网关辅助工具 | 共享审批网关解析辅助工具 |
  | `plugin-sdk/approval-handler-adapter-runtime` | 审批适配器辅助工具 | 面向热通道入口的轻量级原生审批适配器加载辅助工具 |
  | `plugin-sdk/approval-handler-runtime` | 审批处理器辅助工具 | 更宽泛的审批处理器运行时辅助工具；当更窄的 adapter/gateway 接缝足够时优先使用它们 |
  ...
  | `plugin-sdk/security-runtime` | 安全辅助工具 | 共享信任、DM 门控、根边界文件/路径辅助工具、外部内容和秘密收集辅助工具 |
  ...
  | `plugin-sdk/error-runtime` | 错误格式化辅助工具 | `formatUncaughtError`, `isApprovalNotFoundError`, error graph 辅助工具 |
  ...
  | `plugin-sdk/reply-dispatch-runtime` | 窄范围回复分发辅助工具 | finalize、provider 分发和 conversation-label 辅助工具 |
  ...
  | `plugin-sdk/provider-model-shared` | 共享 provider model/replay 辅助工具 | `ProviderReplayFamily`, `buildProviderReplayFamilyHooks`, `normalizeModelCompat`, 共享 replay-policy 构建器, provider-endpoint 辅助工具，以及 model-id 规范化辅助工具 |
  ...
  | `plugin-sdk/provider-web-search-config-contract` | provider web-search 配置辅助工具 | 针对不需要 plugin-enable 接线的 provider 的窄范围 web-search 配置/凭据辅助工具 |
  | `plugin-sdk/provider-web-search-contract` | provider web-search 契约辅助工具 | 窄范围 web-search 配置/凭据契约辅助工具，例如 `createWebSearchProviderContractFields`、`enablePluginInConfig`、`resolveProviderWebSearchPluginConfig`，以及作用域化凭据 setter/getter |
  | `plugin-sdk/provider-web-search` | provider web-search 辅助工具 | web-search provider 注册/缓存/运行时辅助工具 |
  | `plugin-sdk/provider-tools` | provider 工具/schema 兼容辅助工具 | `ProviderToolCompatFamily`, `buildProviderToolCompatFamilyHooks`，以及 DeepSeek/Gemini/OpenAI schema 清理 + 诊断 |
  ...
  | `plugin-sdk/provider-transport-runtime` | provider 传输辅助工具 | 原生 provider 传输辅助工具，如受保护 fetch、传输消息转换和可写传输事件流 |
  | `plugin-sdk/keyed-async-queue` | 有序异步队列 | `KeyedAsyncQueue` |
  | `plugin-sdk/media-runtime` | 共享媒体辅助工具 | 媒体 fetch/transform/store 辅助工具、基于 ffprobe 的视频尺寸探测，以及媒体负载构建器 |
  | `plugin-sdk/media-generation-runtime` | 共享媒体生成辅助工具 | 共享故障转移辅助工具、候选项选择，以及图像/视频/音乐生成的缺失模型消息 |
  ...
  | `plugin-sdk/speech` | 语音辅助工具 | 语音 provider 类型以及面向 provider 的指令、注册表、验证辅助工具，以及 OpenAI 兼容的 TTS 构建器 |
  ...
  | `plugin-sdk/realtime-voice` | 实时语音辅助工具 | provider 类型、注册/解析辅助工具、桥接会话辅助工具、共享 agent 互聊队列、活动运行语音控制、转录/事件健康、回声抑制、咨询问题匹配、强制咨询协调、turn-context 跟踪、输出活动跟踪，以及快速上下文咨询辅助工具 |
  | `plugin-sdk/image-generation` | 图像生成辅助工具 | 图像生成 provider 类型以及图像资产/data URL 辅助工具和 OpenAI 兼容的图像 provider 构建器 |
  ...
  | `plugin-sdk/channel-config-primitives` | 通道配置原语 | 窄范围通道配置 schema 原语 |
  ...
  | `plugin-sdk/direct-dm-guard-policy` | 直接 DM 守卫辅助工具 | 窄范围的预加密守卫策略辅助工具 |
  ...
  | `plugin-sdk/memory-core` | 捆绑 memory-core 辅助工具 | Memory manager/config/file/CLI 辅助工具面 |
  | `plugin-sdk/memory-core-engine-runtime` | Memory engine 运行时外观 | Memory 索引/搜索运行时外观 |
  | `plugin-sdk/memory-core-host-embedding-registry` | Memory embedding 注册表 | 轻量级 memory embedding provider 注册表辅助工具 |
  | `plugin-sdk/memory-core-host-engine-foundation` | Memory host foundation engine | Memory host foundation engine 导出 |
  | `plugin-sdk/memory-core-host-engine-embeddings` | Memory host embedding engine | Memory embedding 契约、注册表访问、本地 provider 以及通用批处理/远程辅助工具；具体远程 provider 位于其所属插件中 |
  ...
  | `plugin-sdk/memory-core-host-events` | 已弃用的 memory 事件别名 | 使用 `plugin-sdk/memory-host-events` |
  ...
  | `plugin-sdk/memory-host-core` | Memory host core 运行时别名 | 面向供应商中立的 memory host core 运行时辅助工具别名 |
  | `plugin-sdk/memory-host-events` | Memory host 事件日志别名 | 面向供应商中立的 memory host 事件日志辅助工具别名 |
  | `plugin-sdk/memory-host-files` | 已弃用的 memory 文件/运行时别名 | 使用 `plugin-sdk/memory-core-host-runtime-files` |
  | `plugin-sdk/memory-host-markdown` | 托管 markdown 辅助工具 | 面向 memory 相邻插件的共享托管 markdown 辅助工具 |
  | `plugin-sdk/memory-host-search` | 活动 memory 搜索外观 | 懒加载的活动 memory 搜索管理器运行时外观 |
  | `plugin-sdk/memory-host-status` | 已弃用的 memory host status 别名 | 使用 `plugin-sdk/memory-core-host-status` |
  | `plugin-sdk/testing` | 测试工具 | 仓库本地已弃用兼容总入口；请使用聚焦的仓库本地测试子路径，例如 `plugin-sdk/plugin-test-runtime`、`plugin-sdk/channel-test-helpers`、`plugin-sdk/channel-target-testing`、`plugin-sdk/test-env` 和 `plugin-sdk/test-fixtures` |
</Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="subagent_spawning hook → core thread binding">
    **旧**：`api.on("subagent_spawning", handler)` returning
    ...
    **新**：让核心通过通道会话绑定适配器准备 `thread: true` 的子 agent 绑定。仅使用 `api.on("subagent_spawned", handler)` 进行启动后的观察。
    ...
    `SubagentLifecycleHookRunner.runSubagentSpawning(...)` 仅在外部插件迁移期间作为已弃用兼容面保留。

  </Accordion>

  <Accordion title="Provider discovery types → provider catalog types">
    四个 discovery 类型别名现在只是 catalog 时代类型的薄包装：
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **新**：单个 `resolveThinkingProfile(ctx)`，返回一个带有标准 `id`、可选 `label` 和分级 level 列表的 `ProviderThinkingProfile`。OpenClaw 会根据 profile 等级自动降级陈旧的存储值。
    ...
    只实现一个 hook，不要实现三个。旧 hook 在弃用窗口期间仍可工作，
    但不会与 profile 结果组合。

  </Accordion>

  <Accordion title="External auth providers → contracts.externalAuthProviders">
    **旧**：在不在插件清单中声明 provider 的情况下实现外部认证 hook。
  ...
  <Accordion title="Memory embedding provider API">
    ...
    **新**：`api.registerEmbeddingProvider(...)` 加上
    ...
    通用 embedding provider 契约可在 memory 之外复用，并且是新 provider 的受支持路径。面向 memory 的注册 API 在现有 provider 迁移期间仍以已弃用兼容方式接线。插件检查会将非捆绑用法报告为兼容性债务。
