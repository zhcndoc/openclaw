...
- `Local`：仅本地模型
- `Cloud + Local`：本地模型加上 Ollama Cloud 模型
- 像 `kimi-k2.5:cloud`、`minimax-m2.5:cloud` 和 `glm-5:cloud` 等云模型**不需要**本地执行 `ollama pull`
...
5. 如果你偏好手动设置，可直接为 OpenClaw 启用 Ollama（任意值均可；Ollama 不要求真实密钥）：
