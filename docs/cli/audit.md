...
对于 Gateway 运行，已解析的经过身份验证的配置文件可以使调用方显示为
`present`，覆盖范围显示为 `attribution-only`。配对设备和共享凭据无法证明某个具体人员：没有持久配置文件时，调用方保持为 absent；如果经过身份验证的用户证据承诺存在某个配置文件但无法解析，则调用方保持为 `unknown`。会话创建会保留当前规范的持久配置文件 id，因此配置文件关联不会使所有权失去归属；而运行检查使用的是连接时不可变的审计事实。普通会话来源信息不会存储显示标签。只有在明确启用该审计存储后，执行身份中才可以保留可选的、有界且已去除机密信息的标签。

终止批准回执会显示 `allowed` 或 `denied`、其稳定的原因代码、强制执行状态、权威来源边界、策略和授权引用、所使用的上下文字段以及修复措施。过期和已取消的批准属于被拒绝的非操作，并具有不同的原因代码。只有当批准所有者记录了终止状态时，`no-route` 才是强制执行的拒绝。损坏的批准为 `unknown`。文本视图将 `operator_approvals` 标记为由所有者原生维护的 SQLite 记录，保留 30 天；JSON 会保留相同的来源所有者和记录引用，不进行有损重格式化。
`enforced` 要求批准的不可变所有者本地绑定与所选上下文、执行和运行完全匹配。缺失、格式错误或不匹配的绑定会分别报告
`operator_approval_execution_link_missing`、`operator_approval_execution_link_malformed` 或
`operator_approval_execution_link_mismatch`，覆盖范围为 unknown，且不包含授权引用。检查器绝不会根据 `runId`、会话元数据、时间戳或保留执行的数量重建该绑定。

JSON 输出是未经有损重格式化的 Gateway 结果。精确结果包含一个有界的 V1 上下文（最大 16 KiB）、最多 100 个决策回执、覆盖范围和缺失证据代码，以及可选的 `nextDecisionCursor`。模糊运行结果则包含最多 50 个执行候选项和可选的 `nextExecutionCursor`。敏感的域、运行时、调用方、保证、入口来源和授权引用均为安装本地的 HMAC 投影。已配置的代理 id 和精确的运行 id 仍然可见，上下文和执行 id 也同样可见，因此重定向的输出仍属于私有操作员数据。
...
一旦上下文超过 30 天，CLI 不会返回其中的任何字段或关联决策。在有界清理待处理期间，结果为 `unsupported`，并提供过期后重新运行的后续步骤。清理完成后，如果没有单独保留的活动，结果可能变为 `unknown`；这种缺失不证明运行未发生。启动时和每小时的维护任务每次最多清理 1,024 个身份上下文，并且即使收集功能已禁用也会继续执行。队列饱和、工作器/存储故障、清理失败或进程意外终止都可能丢失尽力而为的证据，但绝不会阻塞或中止代理运行。只要其写入器生命周期允许，正常的 Gateway 和直接本地 CLI 关闭都会刷新已接受的工作。
...
闭合请求必须且只能指定 `executionId` 或 `runId` 其中之一。
`decisionLimit` 的范围为 1–100，`decisionCursor` 为可选项。运行发现还接受范围为 1–50 的 `executionLimit` 和可选的 `executionCursor`。具有多个保留执行的运行会返回类型化的 `ambiguous` 身份状态，在调用方选择 execution id 之前不会返回身份上下文或决策。对于选定的单个上下文，回执分页会先从准入开始，然后读取所有者原生的终止批准，最后读取没有原生持久记录的边界所对应的通用事实。批准检查绝不会写入重复的通用记录。通用事实写入和投影同样要求完整的上下文、执行和运行元组与不可变执行上下文匹配。

活动账本仍然是尽力而为的。相比之下，返回的批准回执来自权威的先回答者获胜批准行，返回的通用回执来自附加式不可变决策事实表。三个表面都使用 30 天保留期，但活动账本中缺少记录不能证明批准或操作没有发生。在有界工作器写入持久化该行之前，通用事实的交付同样是尽力而为的；所有者原生的批准持久化不使用该队列。
