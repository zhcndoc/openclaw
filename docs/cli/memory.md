...
- `--deep`: 探测向量 + embedding 可用性。普通 `memory status` 保持快速，不会执行实时 embedding ping。QMD 词法 `searchMode: "search"` 会跳过语义向量探测和 embedding 维护，即使使用 `--deep` 也是如此。
- `--index`：如果存储区是脏的，则执行重新索引（隐含 `--deep`）。
