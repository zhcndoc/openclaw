...
快照创建会在读取之前验证活动数据库，使用 SQLite `VACUUM INTO` 将已提交的 WAL 状态捕获到一个紧凑数据库中，再次验证生成的数据库，并在不覆盖现有路径的情况下发布已完成的目录。全局快照会移除临时投递队列行并再次压缩，以便已删除的队列负载不会保留在空闲页中。
...
快照创建在暂存或发布数据库字节之前，会对仓库应用相同的所有者、ACL、祖先以及路径标识检查。
...
状态目录下的 SQLite 数据库会使用 `VACUUM INTO` 进行压缩，因此已删除页面的残留不会进入归档，且不会复制正在使用的 WAL/SHM 文件。需要未提供的、由所有者定义的 SQLite 能力的插件数据库会直接失败，而不会回退到原始页复制。通过工作区备份包含的 SQLite 文件会作为工作区文件复制，不受压缩保证约束。
