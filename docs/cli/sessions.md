...
它会保留会话记录，将会话标记为已归档，并将其从默认的活动会话列表中移除。对于存在活动部署的云工作器会话，Gateway 会先停止工作器，同步其工作区，然后回收环境。如果部署仍在转换中，或已失败但无法证明其环境已被移除，则会话不会被归档；请等待部署状态稳定后重试。主代理会话仍受保护。已经归档的会话会直接成功且不执行任何操作。使用 `--dry-run` 可验证每个键并预览结果，而不会更改会话状态。
