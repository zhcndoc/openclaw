...
QMD 跨代理搜索路径已被移除。内置记忆不会搜索其他代理的记录语料库；每个代理仅搜索其自身配置的记忆和符合条件的同一代理会话来源。如果同一参考材料应由多个代理建立索引，请将有意共享的 Markdown 放入显式共享的 `memory.search.extraPaths` 目录中。有关完整的升级路径，请参阅[从 QMD 迁移](/concepts/memory-builtin#migrating-from-qmd)。
