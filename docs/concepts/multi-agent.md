...
title: 多代理路由
read_when: "您希望在一个网关进程中使用多个彼此隔离的代理（工作区 + 认证）。"
status: active
---

在一个正在运行的 Gateway 中运行多个 _隔离_ 的代理——每个代理都有自己的工作区、状态目录（`agentDir`）和会话历史——以及多个频道账户（例如两个 WhatsApp 账户）。入站消息会通过绑定路由到正确的代理。

这里的 **agent** 指的是完整的每个角色范围：工作区文件、认证配置文件、模型注册表和会话存储。`agentDir` 是磁盘上的状态目录，保存该每代理配置，路径位于 `~/.openclaw/agents/<agentId>/`。**binding** 会将一个频道账户（例如一个 Slack 工作区或一个 WhatsApp 号码）映射到这些代理中的某一个。
...
`sessions_history` 在这里也是更安全的跨会话回忆路径：它返回一个有界、经过清理的视图，而不是原始转录转储。助手回忆会剥离思考标签、`<relevant-memories>` 框架结构、纯文本工具调用 XML 负载（包括 `<tool_call>...</tool_call>`、`<function_call>...</function_call>`、`<tool_calls>...</tool_calls>`、`<function_calls>...</function_calls>` 以及截断的工具调用块）、降级的工具调用框架结构、泄露的 ASCII/全角模型控制令牌，以及在编辑/截断之前格式错误的 MiniMax 工具调用 XML。

主代理凭证**不会**自动共享。切勿在不同代理之间复用 `agentDir`（这会导致认证/会话冲突）。如果您想共享凭证，请将 `auth-profiles.json` 复制到另一个代理的 `agentDir` 中。

技能从每个代理的工作区以及共享根目录（如 `~/.openclaw/skills`）加载，然后在配置时由有效的代理技能允许列表进行过滤。使用 `agents.defaults.skills` 设置共享基线，使用 `agents.list[].skills` 进行每代理替换。请参阅 [技能：每代理与共享](/tools/skills#per-agent-vs-shared-skills) 和 [技能：代理技能允许列表](/tools/skills#agent-skill-allowlists)。
...
如果一个代理需要搜索另一个代理的 QMD 会话记录，请在
`agents.list[].memorySearch.qmd.extraCollections` 下添加
额外集合。仅当每个代理都应继承相同的共享记录集合时，才使用
    ...
    list: [
      {
        ...
        memorySearch: {
          qmd: {
            extraCollections: [{ path: "notes" }], // 解析到工作区内部 -> 集合命名为 "notes-main"
          },
        },
      },
...
额外的集合路径可以在代理之间共享，但当路径位于代理工作区之外时，集合名称
保持明确。工作区内的路径仍限于代理范围，因此每个代理保留自己的记录搜索集。

## 一个 WhatsApp 号码，多人（私聊拆分）
...
- `binding`：通过 `(channel, accountId, peer)`（及可选的公会/团队 ID）将入站消息路由至某个 `agentId`。
...
详见 [多代理沙箱及工具](/tools/multi-agent-sandbox-tools) 获取详细示例。

## 相关

- [通道路由](/channels/channel-routing) — 消息如何路由至代理
- [子代理](/tools/subagents) — 生成后台代理运行
- [ACP 代理](/tools/acp-agents) — 运行外部编码框架
- [在线状态](/concepts/presence) — 代理在线状态及可用性
- [会话](/concepts/session) — 会话隔离及路由
