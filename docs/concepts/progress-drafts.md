...
<Note>
  当 `channels.discord.streaming.mode`/`streamMode` 未设置时，Discord 已经默认使用
  `streaming.mode: "progress"`，因此进度草稿会在那里自动显示，无需任何配置。其他所有频道默认是 `partial`
  或 `off`；完整的按频道默认值表请参见 [Streaming and chunking](/concepts/streaming#channel-mapping)。
</Note>
...
默认从这里开始：一个自动生成的单词标签、5 秒的启动延迟
（或者在第二个工作事件时立即开始）、在有用的工作进行时显示紧凑的进度行，以及
抑制该轮次中旧的独立进度消息。

本页介绍进度草稿体验及其配置选项。有关完整的流式模式矩阵、
按通道的运行时说明，以及旧键迁移，请参见 [流式传输和分块](/concepts/streaming)。
...
| 进度行         | 使用与 `/verbose` 相同的工具图标和详细格式化器的紧凑运行更新。                     |

当代理开始进行有意义的工作并在初始延迟期间保持忙碌状态，或者第二个工作事件立即触发时，标签会出现一次。它位于滚动进度行列表的顶部，因此一旦出现足够多的具体工作行，它就会滚出视野。纯文本回复永远不会显示进度草稿；只有在出现真实工作更新时才会显示一行，例如 `🛠️ Bash: run tests`、`🔎 Web Search: for "discord edit message"` 或 `✍️ Write: to /tmp/file`。
...
当用户更关心“正在发生什么”，而不是看答案文本逐个 token 流式出现时，选择 `progress`；当答案文本本身就是进度信号时，选择 `partial`；当需要更大的预览块时，选择 `block`。在 Discord 和 Telegram 上，`streaming.mode: "block"` 仍然是预览流式输出，而不是普通的块回复投递——请使用 `streaming.block.enabled`（或旧版 `blockStreaming`）来实现。

## 配置标签

进度标签位于 `channels.<channel>.streaming.progress` 下。  
默认的 `label` 是 `"auto"`，它会从 OpenClaw 内置的单词标签池中选择：
...
| --------------- | -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Discord         | 发送一条消息，然后对其进行编辑。        | 默认为 `progress` 模式；当最终文本能放入一条安全预览消息时，会在原处编辑最终文本。                                                             |
| Matrix          | 发送一个事件，然后对其进行编辑。          | 账户级流式配置控制账户级草稿。                                                                                             |
| Microsoft Teams | 在个人聊天中使用原生 Teams 流。 | `streaming.mode: "block"` 会映射到 Teams 的块式传递。                                                                                           |
| Slack           | 原生流或可编辑草稿帖子。  | 需要一个回复线程目标；没有线程目标的顶级私信仍会获得草稿预览帖子和编辑。                                                           |
| Telegram        | 发送一条消息，然后对其进行编辑。        | 如果某条消息落在进度草稿和答案之间，草稿会重新发布到它下方（先发布新内容，再删除旧内容），而不是让客户端滚动跳转。 |
| Mattermost      | 可编辑草稿帖子。                   | 工具活动会折叠到同一个草稿样式帖子中。                                                                                                       |
...
- 如果草稿可以安全地变成最终答案，OpenClaw 会直接在原处编辑它。
- 如果该通道使用原生进度流式传输，OpenClaw 会在原生传输接受最终文本时完成该流。
- 否则（媒体、审批提示、明确的回复目标、分块过多，或者编辑/发送失败），OpenClaw 会通过正常的通道传递路径发送最终答案，而不是覆盖草稿。
