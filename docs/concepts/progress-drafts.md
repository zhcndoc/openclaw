---
summary: "进度草稿：一条可见的进行中消息，会在代理运行时持续更新"
read_when:
  - 配置长时间聊天轮次的可见进度更新
  - 在部分流式、分块和进度流式模式之间进行选择
...
🔎 网络搜索：查询 "discord edit message"
🛠️ Bash：运行测试
```

<Note>
  Discord 默认将预览流式传输设为 `off`；设置 `streaming.mode: "progress"`
  即可启用。Telegram 默认使用 `progress`，无需额外配置。在任一平台上设置
  `mode: "partial"`，即可改为流式传输答案文本。有关每个频道默认设置的完整表格，请参阅
  [流式传输与分块](/concepts/streaming#channel-mapping)。
</Note>
...
默认设置为：延迟 1.5 秒开始，在执行有用工作期间显示紧凑的进度行，并在本轮中抑制较早的独立进度消息。原始工具行草稿会自动使用一个单词的标签；状态标题会省略这个多余的标题，除非你明确配置了一个标题。
...
例如 `🛠️ Bash: 运行测试`、`🔎 Web Search: 搜索“discord edit message”`，
或 `✍️ Write: 写入 /tmp/file`。

当通道可以安全地这样做时，最终答案会直接替换草稿；否则 OpenClaw 会通过正常传递发送最终答案，并清理或停止更新草稿（参见 [最终处理](#finalization)）。
...
| 频道            | 进度传输方式                               | 说明                                                                                                                                                     |
| --------------- | -------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Discord         | 发送一条消息，然后编辑该消息。        | `progress` 是显式选择启用；最终答案会附带 `-#` 活动回执，并在答案发送后删除状态草稿。                   |
| Matrix          | 发送一个事件，然后编辑该事件。        | 账户级流式传输配置控制账户级草稿。                                                                                             |
| Microsoft Teams | 个人聊天中使用原生 Teams 流。 | `streaming.mode: "block"` 则改为使用 Teams 分块传递。                                                                                           |
| Slack           | 原生流或可编辑的草稿帖子。           | 需要回复线程目标；没有线程目标的顶层私信仍会收到草稿预览帖子，并对其进行编辑。                                                           |
| Telegram        | 发送一条消息，然后编辑该消息。        | 如果在进度草稿和答案之间有消息到达，草稿会重新发布到该消息下方（先发布新消息，再删除旧消息），而不是导致客户端滚动跳跃。 |
| Mattermost      | 可编辑的草稿帖子。                   | `block` 模式会在已完成文本和工具活动帖子之间轮换；其他模式会将工具活动折叠到同一个草稿样式的帖子中。                       |

不支持安全编辑的频道会回退到输入指示器或仅最终内容传递。有关每个频道完整运行时行为分解，请参见 [流式传输和分块](/concepts/streaming)。
