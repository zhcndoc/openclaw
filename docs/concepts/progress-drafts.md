...
默认设置如下：延迟 5 秒后开始（或者在第二个工作事件时立即开始）、在有实际工作进行时显示紧凑的进度行，以及在该轮中抑制较早的独立进度消息。原始工具行草稿会自动使用一个单词的标签；叙述式状态会省略那个重复的标题，除非你显式配置一个。
...
对于原始工具进度，标签会在代理开始有意义的工作后出现，
并在初始延迟期间保持忙碌，或者在第二个工作事件立即触发时出现。
它位于滚动进度行列表的顶部，因此一旦
出现足够多的具体工作行，它就会滚出可视区域。叙述式进度只显示代理的
纯语言状态，除非显式配置了标签。纯文本回复
永远不会显示进度草稿；只有在真正的工作更新时才会出现一行，
...
原始工具行标签是 `"auto"`，它会从 OpenClaw 内置的
单词标签池中选择。叙述式进度会隐藏该隐式标签；如果你希望它也显示在叙述上方，请显式设置 `label: "auto"`：
...
当某个实用模型为代理完成解析时——无论是显式的 [`utilityModel`](/gateway/config-agents#utilitymodel)，还是主提供方声明的小模型默认值（OpenAI → `gpt-5.6-luna`，Anthropic → `claude-haiku-4-5`）——进度草稿会用一段简短的自然语言叙述来替换滚动的工具行，这段叙述由更便宜的模型生成，并会随着工作推进而刷新：

```text
正在更新配置中的默认模型，然后重启网关以使其生效。一次代理列表调用失败，正在重试。
```

叙述默认开启（`streaming.progress.narration`，默认 `true`），且永远不会回退到主模型：它只会在显式 `utilityModel` 或提供方为代理主提供方声明的默认值时运行。将 `utilityModel: ""` 可完全禁用实用路由。工具行会继续在下方累积，并在叙述停止时恢复显示；只有在正常活动门控之后且叙述文本实际变化时，草稿才会被编辑，这可避免快速轮次中的闪烁并减少繁忙通道中的编辑抖动。若要保留原始工具行，请将其禁用：
