...
- **当你希望不同地区的主机之间保持稳定的时间戳时，使用 UTC 信封**（`envelopeTimezone: "utc"`），或者当你希望 UTC 对齐的日志与诊断输出一致时，也应使用它。
- **当网关主机位于一个时区，而用户位于另一个时区，并且你希望信封始终按用户时区显示，而不受主机迁移影响时，使用固定的 IANA 时区**（例如 `"Europe/Vienna"`）。
- **当时间戳上下文对对话没有帮助时，设置 `envelopeTimestamp: "off"`**。这会从信封、直接代理提示前缀以及嵌入的模型输入前缀中移除绝对时间戳。
