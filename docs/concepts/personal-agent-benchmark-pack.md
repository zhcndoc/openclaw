...
- 通过本地 cron 投递伪造个人提醒
- 通过 `qa-channel` 伪造 DM 和线程回复路由
- 从临时 QA 工作区内存文件中伪造偏好回忆
- 伪造机密不回显检查
- 在一个简短的审批式回合后进行安全的基于读取的工具后续执行

## 场景

机器可读的包元数据位于 `extensions/qa-lab/src/scenario-packs.ts`。初始版本未添加 CLI 包选择器，所以请显式运行这些场景：
...
该包面向 `qa-channel` 搭配 `mock-openai` 或其他本地 QA 提供方通道。它不应指向真实聊天服务或真实个人账号。
