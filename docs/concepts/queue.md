...
- `runEmbeddedAgent` 按 **会话键**（lane `session:<key>`）入队，以确保每个会话同一时间只有一个活跃运行。
- 然后，每个会话运行都会进入一个 **全局 lane**（默认 `main`），从而将整体并行度限制为 `agents.defaults.maxConcurrent`。
- 启用详细日志时，排队运行如果在启动前等待超过约 2 秒，会输出一条简短提示。
...
- `steer`: 将消息注入到活跃运行时中。OpenClaw 会在当前助手轮次执行完其工具调用之后、下一次 LLM 调用之前交付所有待处理的引导消息；Codex app-server 会接收一个批量的 `turn/steer`。如果运行未在主动流式输出，或者引导不可用，OpenClaw 会等到活跃运行结束后再开始处理提示。
