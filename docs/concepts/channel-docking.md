...
频道停靠就像是为一个 OpenClaw 会话设置呼叫转移。它会保持相同的
会话上下文，但会改变该会话未来回复的发送位置。
停靠只能从直接聊天中执行；不能在群聊中运行。
...
如果 Alice 从 Telegram 直接聊天中发送此消息：
...
2. 切换到 Discord，在那里协调工作。
3. 在 Telegram 私聊中发送 `/dock_discord`。
4. 保持相同的 OpenClaw 会话，但未来的回复会在 Discord 中接收。
...
OpenClaw 会为每个已加载的、支持原生命令的频道插件生成一个 `/dock-<channel>` 命令，因此随着插件的增加，这个列表也会增长。目前捆绑的、支持该功能的插件有：
...
下划线形式也是 Telegram 等直接暴露斜杠命令的界面上的原生命令名称。
...
请从与 OpenClaw 的直接聊天中发送停靠命令，不要从群聊中发送。
