...
| Kind                   | 记录时间                                             | 通知观察者 |
| ---------------------- | -------------------------------------------------------- | ----------------- |
| `human_direct_message` | 人类直接向被监视的会话发送一个回合       | 是               |
| `upstream_missing`     | 被采纳会话的上游来源消失          | 是               |
| `goal_changed`         | 会话的目标状态被创建、更新或清除 | 是               |
| `child_spawned`        | 创建了一个子代理或 ACP 子会话              | 否（种子光标） |
| `run_completed`        | 子运行成功结束                            | 否（仅日志）     |
| `run_failed`           | 子运行失败、超时或被取消            | 否（仅日志）     |
| `compacted`            | 会话历史被压缩                       | 否（仅日志）     |
| `adopted`              | 目录会话被采纳到 OpenClaw 中               | 否（仅日志）     |
