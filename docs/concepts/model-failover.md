...
通过 `/model …@<profileId> -s` 手动选择会设置一个**用户覆盖项**。有效的用户固定配置文件会在 `/new`、`/reset`、会话轮换、压缩和冷却窗口期间保留。当配置文件消失、不再匹配所选提供方，或用户选择了其他显式配置文件时，OpenClaw 会清除该固定配置。`/model default -s` 会清除模型覆盖项，同时保留兼容的认证固定配置，并清除不兼容的认证固定配置。

<Note>
自动固定的配置文件（由会话路由器选择）会被视为一种**偏好**：它们会被优先尝试，但 OpenClaw 可能会在触发速率限制/超时时轮换到另一个配置文件。当原始配置文件再次可用时，新的运行可以再次优先使用它，而无需更改所选模型或运行时。用户固定的配置文件会在符合条件的同一提供方候选配置文件中保持锁定。配置的默认模型上保留的固定配置仍可通过已配置的模型回退项进行切换；而显式的用户模型选择则保持严格模式，并会直接报告失败。
</Note>
