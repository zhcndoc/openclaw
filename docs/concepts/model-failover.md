...
<Steps>
  ...
  <Step title="耗尽时抛出 FailoverError">
    如果每个候选都失败，则抛出一个 `FailoverError`，其中包含每次尝试的结构化详细信息，以及在已知时最早的冷却期结束时间。
  </Step>
</Steps>
...
启用后，OpenClaw 会在某个非主回退候选项发生认证类失败后，记录一个会话范围内、存储于内存中的跳过标记。该键包含会话、提供商、模型以及所选的自动或显式配置文件 ID。切换配置文件不会继承其他配置文件的失败标记。主候选项永远不会被跳过，因此显式选择模型时仍会显示实际的认证错误。缓存仅限当前进程，Gateway 重启后会清除。
...
这些通知是运行消息，而不是助手内容。它们会在每次状态更改时发送一次，包括在可行的情况下仅产生副作用的轮次，但同一轮次内重复的回退转换不会重复发送。发送会绕过正常的源回复抑制，不会占用线程式频道中的首个助手回复位置，并且不会包含在文本转语音中。
...
OpenClaw **会为每个会话固定自动选择的认证配置文件**，以保持提供方缓存处于预热状态。它**不会在每次请求时轮换**。自动固定的配置文件可能会在以下情况下轮换或清除：

- 会话被重置（`/new` / `reset`）
- 完成一次压缩（压缩计数递增）
- 该配置文件处于冷却/禁用状态

通过 `/model …@<profileId> -s` 手动选择会设置一个**用户覆盖**。有效的用户固定配置文件会在 `/new`、`/reset`、会话轮换、压缩和冷却窗口期间保留。只要该配置文件符合条件，它就仍是首选；当该确切配置文件处于冷却或禁用状态时，OpenClaw 会尝试下一个符合条件的同提供商配置文件，而不会替换已存储的固定配置。配置文件消失、不再匹配所选提供方，或用户选择了其他显式配置文件时，OpenClaw 会清除该固定配置。`/model default -s` 会清除模型覆盖，同时保留兼容的认证固定配置，并清除不兼容的固定配置。

<Note>
自动固定和用户固定的认证配置文件都是重试首选项：只要所选配置文件符合条件，OpenClaw 就会先尝试它，然后可能因认证失败、速率限制、计费限制或超时而轮换到另一个同提供商配置文件。用户固定配置会在临时轮换期间保持持久化，因此冷却期结束后新的运行会再次优先使用它，而不会更改所选模型或运行时。此认证轮换不会放宽模型选择：显式的用户提供方／模型选择在其同提供商认证配置文件耗尽后仍会严格报告失败。
</Note>
...
使用用户固定的配置文件，可以让某个账户／密钥成为该会话中持久的首选。如果它变得不可用，OpenClaw 会暂时轮换遍历其余符合条件的 `auth.order.openai` 配置文件，并在恢复后返回固定的配置文件。

## 冷却

当一个配置文件由于认证/速率限制错误（或看起来像速率限制的超时）而失败时，OpenClaw 会将其标记为冷却并移动到下一个配置文件。

CLI 支持的运行时只有在其恢复、分叉和新会话恢复尝试完成后，才会确定配置文件的健康状态。终端凭据失败会在模型回退前冷却确切选中的配置文件；成功运行会清除过期的失败状态。转录、格式、上下文、提供方调用前超时，以及没有选中配置文件的 CLI 环境失败，都不会更改共享的配置文件健康状态。
...
当每个候选项都失败时，OpenClaw 会抛出带有结构化尝试记录的 `FailoverError`。外层回复运行器可以使用这些记录来构建更具体的消息，例如“所有模型暂时都受到速率限制”，并在已知最早冷却到期时间时将其包含在内。
