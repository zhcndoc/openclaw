...
<AccordionGroup>
  <Accordion title="3. Legacy state migrations (disk layout)">
    ...
    Divergent or unverified copies remain in place. Doctor reports the owner and an explicit quarantine command for an operator who has inspected the data and stopped OpenClaw. Independent state migrations still run; repairs requiring the refused database are skipped. Doctor exits unsuccessfully and Gateway startup remains blocked until the ownership conflict is resolved. [Per-agent startup isolation](https://github.com/openclaw/openclaw/issues/144689) is separate work.
