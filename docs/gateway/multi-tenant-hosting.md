---
summary: "将多个租户信任域作为每个租户一个彼此隔离的 OpenClaw Gateway cell 进行托管"
read_when:
...
Fleet 目前处于**实验性**阶段：在此表面稳定之前，其命令、标志和容器配置文件可能会在各个版本之间发生变化，且不会提供弃用窗口。
...
Fleet CLI 是一个宿主机侧的生命周期监督器。它会将 cell 记录到 OpenClaw 状态数据库中，并要求本地 Docker 或 Podman 运行时去创建、检查、启动、停止、替换和移除它们的容器。远程运行时端点会被拒绝，因为 Fleet 的绑定路径和回环 URL 属于本地主机；远程 cell 主机会被暂缓处理，直到它们具有明确的存储和端点契约。Fleet 不会代理租户消息，也不会在 cell 之间添加共享的应用层数据路径。
...
持久化的 Gateway 状态来自 `<state-dir>/fleet/cells/<tenant>/`，并挂载到 `/home/node/.openclaw`。auth-profile 加密密钥来自独立的 `<state-dir>/fleet/auth-profile-secrets/<tenant>/` 宿主机路径，并挂载到 `/home/node/.config/openclaw`，与官方 [Docker 持久化布局](/install/docker#storage-and-persistence) 保持一致。该密钥不会嵌套在普通状态挂载之下。每个租户的通道账户都终止于拥有它们的 cell 内，因此在 Fleet MVP 中不存在共享通道账户或共享入站消息路由器。
...
1. **加固的容器基线。** Fleet 会移除所有 Linux capabilities，启用 `no-new-privileges`，应用 PID、内存、CPU 以及可选的可写层磁盘限制，使用独立的持久化挂载和每个 cell 的网络，并且仅发布到主机回环地址。桥接网络不会限制出站流量；当某个 cell 不能发起外部连接时，请使用 Podman `--network internal` 或 Docker 主机防火墙策略。这是适用于信任运营者和主机的租户的 MVP 配置。
2. **更强的容器或虚拟机隔离。** 对于风险更高的工作负载，请将 Docker 或 Podman 配置为使用更强的 OCI 隔离运行时，例如 gVisor 或 Kata Containers，或者将 cells 放入 microVM 中。这属于运行时或基础设施配置；Fleet 的 `--runtime docker|podman` 选项选择的是容器 CLI，而不是 OCI 隔离后端。参见 Docker 的 [替代容器运行时](https://docs.docker.com/engine/daemon/alternative-runtimes/) 和 [Docker VM 运行时指南](/install/docker-vm-runtime)。
3. **为不可信租户使用独立机器。** 不要将不可信租户共置在同一个 OpenClaw 进程或 OS 用户中。当租户不信任同一个主机运营者，或者需要更强的管理边界时，请使用具有独立运行时管理的独立虚拟机或物理主机。

这一级阶梯中的任何一层都不会改变 OpenClaw 应用的信任模型：一个 Gateway 仍然是一个受信任的运营者域。
...
这些功能需要明确的身份、路由、授权和故障域契约。它们不应通过在租户之间共享一个 Gateway 或其凭据来近似实现。它们也不属于 Fleet 的职责范围：Fleet 保持为单主机生命周期监督器，而多机器、由身份治理的 fleet 则属于其之上的专用控制平面层。
