...
- 前台运行会直接返回保留的输出，并在此前的输出超过总上限时进行说明。
- 在后台运行时（显式指定或因 `yieldMs` 超时），工具会返回 `status: "running"`、`sessionId` 和一小段输出尾部。
- 后台运行和通过 `yieldMs` 启动的运行会继承 `tools.exec.timeoutSeconds`，除非调用时显式传入 `timeout`。
- 输出会一直保存在内存中，直到会话被轮询或清除，但不会超过每个会话的总上限。
- 已完成的会话会在配置的 TTL 到期后失效。注册表最多还会保留 50 个已完成会话和总计 2,000,000 个已保留的输出字符，并优先驱逐最旧的记录。即使最新完成的会话记录单独就超过全局上限，也会保留其达到上限的每会话输出。
- 如果不允许使用 `process` 工具，`exec` 将同步运行，并忽略 `yieldMs`/`background`。
- 派生的 exec 命令会接收 `OPENCLAW_SHELL=exec`，以便应用上下文感知的 Shell/配置文件规则。
- 对于立即启动的长时间运行任务：启动一次，依靠自动完成唤醒（启用时）在命令输出内容或失败后进行处理。
- 如果自动完成唤醒不可用，或者需要确认某个命令在无输出的情况下成功退出，请使用 `process` 进行轮询。
- 不要使用 `sleep` 循环或反复轮询来模拟提醒或延迟后续操作；未来任务请使用 cron。
