...
A failed placement does not always mean its worker has stopped. The sidebar, session list, and placement chip keep **Stop cloud worker…** available when cleanup is still needed. Once the Gateway confirms the worker is gone, those menus hide Stop and the placement chip offers **Restart session…**. The failure reason remains visible; an archived session must be unarchived before continuing.
