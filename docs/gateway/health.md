...
Gateway 暴露三个无需身份验证的 `GET`/`HEAD` 探测对：

| 端点                    | 含义                                                                                                        | 用途                                                         |
| ----------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| `/health`、`/healthz`   | HTTP 服务器处于运行状态。                                                                                   | 进程存活性和重启决策。                                       |
| `/startup`、`/startupz` | 启动工作已完成，且 Gateway 未处于排空状态。不检查频道健康状况。                                             | 编排器启动和流量接入。                                       |
| `/ready`、`/readyz`     | 启动已完成，Gateway 未处于排空状态，且已配置的频道账户通过深度就绪检查。                                    | 应显示严重频道故障的操作员监控。                             |

`/startupz` 在启动 sidecar 仍处于等待状态时返回 `503`，并将 `status` 设置为 `"starting"`；在排空期间返回 `503`，并将 `status` 设置为 `"draining"`；其他情况下返回 `200`，并将 `status` 设置为 `"started"`。将其用于 Kubernetes、Fly、Render 以及类似的流量接入场景。损坏的 Telegram 或其他频道账户可能会使 `/readyz` 返回 `503`，但不会通过 `/startupz` 影响正常运行的 Control UI。

远程未经过身份验证的启动响应只包含 `ok` 和 `status`。本地直接调用方和经过身份验证的调用方还会在启动等待期间收到 `version`、`uptimeMs` 和 `pendingReason`。就绪状态详情遵循相同的本地或经过身份验证的访问控制，因为其中可能会指出发生故障的子系统。
