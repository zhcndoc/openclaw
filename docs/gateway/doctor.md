...
- 技能状态摘要（合格/缺失/阻塞）。
- 旧值配置规范化。
- 浏览器迁移检查，用于遗留 Chrome 扩展配置和 Chrome MCP 准备情况。
- OpenCode 提供者覆盖警告（`models.providers.opencode` / `models.providers.opencode-go`）。
- 旧版磁盘状态迁移（会话/agent 目录/WhatsApp 认证）。
- 旧版定时任务存储迁移（`jobId`，`schedule.cron`，顶层 delivery/payload 字段，payload `provider`，简单 `notify: true` webhook 兼容任务）。
- 状态完整性和权限检查（会话，转录，状态目录）。
- 本地运行时配置文件权限检查（chmod 600）。
- 模型认证健康检查：检查 OAuth 过期，尝试刷新即将过期的令牌，报告认证配置的冷却/禁用状态。
- 额外工作区目录检测（`~/openclaw`）。
- 开启沙箱时沙箱镜像修复。
- 旧版服务迁移及额外网关检测。
...
- 网关端口冲突诊断（默认端口 `18789`）。
- 开放 DM 策略的安全警告。
- 本地令牌模式的网关认证检查（无令牌源时提供生成令牌，不覆盖 SecretRef 配置）。
...
网关在启动时检测到旧配置格式时也会自动运行 doctor 迁移，因此过时配置可自动修复，无需手动操作。
...
此检查**不适用于** Docker、沙箱、远程浏览器或其他无头流程，它们继续使用原始 CDP。
...
这些迁移为最佳努力且幂等操作；doctor 会在留下旧目录备份时发出警告。网关和 CLI 启动时也会自动迁移旧版 session 和 agent 目录，使历史、认证和模型归入对应代理路径，无需手动执行 doctor。WhatsApp 认证迁移仅通过 `openclaw doctor` 触发。
...
Doctor 检查认证存储的 OAuth 配置文件，警告令牌即将或已经过期，且在安全条件下尝试刷新它们。若 Anthropic Claude Code 配置已过期，建议运行 `claude setup-token`（或粘贴 setup-token）。刷新过程仅在交互（TTY）模式提示，`--non-interactive` 参数跳过刷新。

Doctor 还会报告导致配置文件暂不可用的状态：
...
Doctor 打印当前工作区的合格、缺失、阻塞中技能的快速摘要。
