...
- `every`: duration string (ms/s/m/h). 默认：`30m`。
    ...
    entries: {
      openshell: {
        ...
        config: {
          ...
          policy: "strict", // optional OpenShell 策略 ID
...
- `id`：稳定的代理 ID（必填）。
- `default`：多代理时，第一个为默认代理（会记录警告）；若未设置，则第一个列表成员为默认。
- `model`：字符串形式覆盖仅主模型；对象形式 `{ primary, fallbacks }` 完全替代（空数组可禁用全局后备）。只覆盖主模型的定时任务依旧继承默认后备，除非设置 `fallbacks: []`。
- `params`：代理专用参数覆盖，合并至默认模型目录对应项。用于设置缓存寿命、温度、最大令牌等，无需复制全模型目录。
- `runtime`：可选代理运行时描述符。`type: "acp"` 搭配 `runtime.acp`（默认字段：`agent`、`backend`、`mode`、`cwd`），用于默认 ACP 会话配置。
- `identity.avatar` 支持工作区相对路径、本地/远程 URL，或 `data:` URI。
- `identity` 自动派生默认值：`ackReaction`（根据 emoji）、`mentionPatterns`（由 name/emoji 派生）。
...
- 发现支持本地 OpenClaw 插件以及兼容的 Codex 包和 Claude 包，包括无清单的 Claude 默认布局包。
- **配置变更需要重启网关。**
- `allow`：可选允许列表（仅加载列出的插件）。`deny` 优先。
- `plugins.entries.<id>.apiKey`：插件级 API 密钥便捷字段（当插件支持时）。
- `plugins.entries.<id>.env`：插件作用域环境变量映射。
- `plugins.entries.<id>.hooks.allowPromptInjection`：设置为 `false` 时，核心阻止 `before_prompt_build`，忽略来自旧 `before_agent_start` 的提示修改字段，同时保留旧的 `modelOverride` 和 `providerOverride`。适用于本地插件钩子和支持的包提供钩子目录。
- `plugins.entries.<id>.config`：插件定义的配置对象（如有本地 OpenClaw 插件架构则验证）。
- 启用的 Claude 包插件还可以从 `settings.json` 提供嵌入式 Pi 默认设置；OpenClaw 作为消毒后的代理设置应用，而非直接作为 raw OpenClaw 配置修改。
- `plugins.slots.memory`：选择激活的内存插件 ID，或 `"none"` 禁用内存插件。
- `plugins.slots.contextEngine`：选择激活的上下文引擎插件 ID；默认为 `"legacy"`，除非安装并选择其他引擎。
- `plugins.installs`：CLI 管理的安装元数据，用于 `openclaw plugins update`。
...
- 可选强化头信息：
  - `gateway.http.securityHeaders.strictTransportSecurity`（仅针对受托 HTTPS 源，详见[Trusted Proxy Auth](/gateway/trusted-proxy-auth#tls-termination-and-hsts)）
