...
OpenProse 是一种可移植、以 Markdown 为先的工作流格式，用于编排 AI 会话。在 OpenClaw 中，它以插件形式提供，安装后会带来一个 OpenProse 技能包以及一个 `/prose` 斜线命令。程序存放在 `.prose` 文件中，并且可以通过显式控制流启动多个子代理。
...
对于确定性、受审批门控的工作流，可参考 [Lobster](/tools/lobster)。
