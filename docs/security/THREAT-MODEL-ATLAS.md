...
**Version:** 1.0-draft | **Framework:** [MITRE ATLAS](https://atlas.mitre.org/)（针对 AI 系统的对抗性威胁环境）+ 数据流图
...
**关键 ATLAS 资源：** [技术](https://atlas.mitre.org/techniques/) | [战术](https://atlas.mitre.org/tactics/) | [案例研究](https://atlas.mitre.org/studies/) | [ATLAS GitHub](https://github.com/mitre-atlas/atlas-data) | [为 ATLAS 做贡献](https://atlas.mitre.org/resources/contribute)
...
| **Attack vector**       | 提示注入结合 exec 审批绕过                                                  |
| **Affected components** | Bash 工具、命令执行                                                                         |
| **Current mitigations** | Exec 审批、Docker 沙箱选项（默认运行时后端）                                      |
| **Residual risk**       | 严重 - 在禁用沙箱时可能进行主机执行                                          |
| **Recommendations**     | 改进审批 UX；关闭沙箱的部署属于有意的操作员选择，并已如此记录 |
...
| **Attack vector**       | 自动化消息洪泛、昂贵的工具调用   |
| **Affected components** | 网关、代理会话、API 提供方              |
| **Current mitigations** | 无                                               |
| **Residual risk**       | 高 - 没有按发送者的速率限制                 |
| **Recommendations**     | 按发送者设置速率限制、成本预算               |
...
| ----------------------- | ----------------------------------------------------------- |
| **ATLAS ID**            | AML.T0031 - 破坏 AI 模型完整性                        |
| **Description**         | 攻击者使代理发送有害/冒犯性内容 |
| **Attack vector**       | 诱导产生不当响应的提示注入            |
| **Affected components** | 输出生成、通道消息发送                        |
| **Current mitigations** | LLM 提供方内容策略                               |
| **Residual risk**       | 中 - 提供方过滤并不完美                     |
| **Recommendations**     | 输出过滤层、用户控制                       |
