...
OpenClaw 支持三种迁移路径：从另一个代理系统导入、将现有安装移动到新机器，以及就地升级插件。

## 从另一个代理系统导入

使用随附的迁移提供程序将说明、MCP 服务器、技能、模型配置，以及（可选）API 密钥导入到 OpenClaw 中。在任何更改之前都会预览计划，报告中的敏感信息会被隐藏，并且应用操作由经过验证的备份提供支持。
...
<CardGroup cols={2}>
  ...
  <Card title="从 Hermes 迁移" href="/install/migrating-hermes" icon="feather">
    导入 Hermes 配置、提供程序、MCP 服务器、内存、技能和受支持的 `.env` 键。
  </Card>
</CardGroup>

CLI 入口点是 [`openclaw migrate`](/cli/migrate)。当 onboarding 检测到已知来源时，也可以提供迁移流程（`openclaw onboard --flow import`）。
...
- **认证** — 每个代理的 `auth-profiles.json`（API 密钥以及 OAuth），以及 `credentials/` 下的任何通道或提供程序状态。
- **会话** — 对话历史和代理状态。
- **通道状态** — WhatsApp 登录、Telegram 会话及类似内容。
...
<Steps>
  ...
  <Step title="在新机器上安装 OpenClaw">
    在新机器上[安装](/install) CLI（以及需要时的 Node）。如果 onboarding 创建了一个新的 `~/.openclaw/`，也没有问题。接下来你会将其覆盖。
  </Step>
...
<AccordionGroup>
  <Accordion title="配置文件或状态目录不匹配">
    如果旧网关使用了 `--profile` 或 `OPENCLAW_STATE_DIR`，而新网关没有使用这些设置，通道将显示为已登出，且会话将为空。使用你迁移的 **相同** 配置文件或状态目录启动网关，然后重新运行 `openclaw doctor`。
  </Accordion>

  <Accordion title="只复制 openclaw.json">
    单独的配置文件是不够的。模型认证配置文件位于 `agents/<agentId>/agent/auth-profiles.json`，而通道和提供程序状态位于 `credentials/` 下。务必迁移 **整个** 状态目录。
  </Accordion>

  <Accordion title="权限和所有权">
    如果你是以 root 身份复制的，或者切换了用户，网关可能无法读取凭据。请确保状态目录和工作区由运行网关的用户拥有。
  </Accordion>

  <Accordion title="远程模式">
    如果你的 UI 指向一个 **远程** 网关，那么远程主机负责会话和工作区。应迁移网关主机本身，而不是你的本地笔记本电脑。参见[FAQ](/help/faq#where-things-live-on-disk)。
  </Accordion>

  <Accordion title="备份中的密钥">
    状态目录包含认证配置文件、通道凭据和其他提供程序状态。请将备份加密存储，避免不安全的传输渠道，并在怀疑泄露时轮换密钥。
  </Accordion>
</AccordionGroup>
...
就地插件升级会保留相同的插件 id 和配置键，但可能会将磁盘上的状态移动到当前布局中。插件特定的升级指南与其通道放在一起：
