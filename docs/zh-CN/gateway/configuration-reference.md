---
description: ~/.openclaw/openclaw.json 的字段级完整参考
read_when:
  - 你需要精确到字段级别的配置语义或默认值
...
- `seamColor`：原生应用 UI 外观的强调色（对话模式气泡着色等）。
...
- `port`：用于 WS + HTTP 的单一复用端口。优先级顺序：`--port` > `OPENCLAW_GATEWAY_PORT` > `gateway.port` > `18789`。
- `bind`：`auto`、`loopback`（默认）、`lan`（`0.0.0.0`）、`tailnet`（仅 Tailscale IP）或 `custom`。
- **旧版 bind 别名**：请在 `gateway.bind` 中使用 bind 模式值（`auto`、`loopback`、`lan`、`tailnet`、`custom`），不要使用主机别名（如 `0.0.0.0`、`127.0.0.1`、`localhost`、`::`、`::1`）。
- **Docker 说明**：默认的 `loopback` 绑定在容器内监听 `127.0.0.1`。Docker 桥接网络（`-p 18789:18789`）中流量从 `eth0` 进入，导致网关不可达。请使用 `--network host`，或设置 `bind: "lan"`（或 `bind: "custom"` 并配合 `customBindHost: "0.0.0.0"`）以监听所有接口。
- **认证**：默认要求认证。非 loopback 绑定时需要共享 token/password。新手引导默认会生成一个 token。
- 如果同时配置了 `gateway.auth.token` 和 `gateway.auth.password`（包括 SecretRef），请显式设置 `gateway.auth.mode` 为 `token` 或 `password`。若两者都配置但 mode 未设置，启动和服务安装/修复流程都会失败。
- `gateway.auth.mode: "none"`：显式无认证模式。仅用于受信任的本地 loopback 设置；新手引导中故意不提供此选项。
- `gateway.auth.mode: "trusted-proxy"`：将认证委托给具备身份感知能力的反向代理，信任来自 `gateway.trustedProxies` 的身份头（见 [Trusted Proxy Auth](/gateway/trusted-proxy-auth)）。
- `gateway.auth.allowTailscale`：为 `true` 时，Tailscale Serve 身份头可满足 Control UI/WebSocket 认证（通过 `tailscale whois` 验证）；HTTP API 端点仍要求 token/password 认证。此无 token 流程假定网关主机受信任。当 `tailscale.mode = "serve"` 时默认值为 `true`。
- `gateway.auth.rateLimit`：可选的认证失败限流器。按客户端 IP 和认证范围生效（共享密钥和设备 token 分别跟踪）。被阻止的尝试返回 `429` + `Retry-After`。
  - `gateway.auth.rateLimit.exemptLoopback` 默认值 `true`；如果希望 localhost 流量也受限流（用于测试或严格代理部署），请改为 `false`。
- 来自浏览器 origin 的 WS 认证尝试始终会在禁用 loopback 豁免时限流（作为对基于浏览器的 localhost 暴力破解的纵深防御）。
- `tailscale.mode`：`serve`（仅 tailnet，loopback 绑定）或 `funnel`（公开，需认证）。
- `controlUi.allowedOrigins`：用于 Gateway 网关 WebSocket 连接的显式浏览器 origin 白名单。非 loopback origin 的浏览器客户端必须设置。
- `controlUi.dangerouslyAllowHostHeaderOriginFallback`：危险选项，启用依赖 Host-header origin 策略的部署的 Host-header origin 回退。
- `remote.transport`：`ssh`（默认）或 `direct`（ws/wss）。当为 `direct` 时，`remote.url` 必须是 `ws://` 或 `wss://`。
- `OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`：客户端紧急覆盖，允许对受信任的私有网络 IP 使用明文 `ws://`；默认仍仅支持 loopback 明文。
- `gateway.remote.token` / `.password` 是远程客户端凭证字段，本身不配置网关认证。
- `gateway.push.apns.relay.baseUrl`：官方/TestFlight iOS 版本在将基于 relay 的注册发布到网关后，供网关使用的外部 APNs relay 基础 HTTPS URL。该 URL 必须与 iOS 构建中编译的 relay URL 一致。
- `gateway.push.apns.relay.timeoutMs`：网关到 relay 的发送超时时间（毫秒），默认 `10000`。
- 基于 relay 的注册会委派给特定网关身份。配对的 iOS 应用获取 `gateway.identity.get`，将身份包含到 relay 注册并转发带注册作用域的发送授权给网关。其他网关无法复用该已存储注册。
- `OPENCLAW_APNS_RELAY_BASE_URL` / `OPENCLAW_APNS_RELAY_TIMEOUT_MS`：覆盖上述 relay 配置的临时环境变量。
- `OPENCLAW_APNS_RELAY_ALLOW_HTTP=true`：仅开发用逃生口，用于 loopback HTTP relay URL。生产环境 relay URL 应保持为 HTTPS。
- `gateway.channelHealthCheckMinutes`：渠道健康检查监视间隔（分钟）。设为 `0` 可全局禁用健康检查重启。默认值：`5`。
- `gateway.channelStaleEventThresholdMinutes`：陈旧 socket 阈值（分钟）。应大于或等于 `gateway.channelHealthCheckMinutes`。默认：`30`。
- `gateway.channelMaxRestartsPerHour`：滚动一小时内，每个渠道/账户允许的最大健康检查重启次数。默认：`10`。
- `channels.<provider>.healthMonitor.enabled`：按渠道选择性关闭健康检查重启，保留全局监视启用。
- `channels.<provider>.accounts.<accountId>.healthMonitor.enabled`：多账户渠道的按账户覆盖，优先于渠道级。
- 仅当 `gateway.auth.*` 未设置时，本地 Gateway 网关调用路径可回退到 `gateway.remote.*`。
- 如果 `gateway.auth.token` / `gateway.auth.password` 通过 SecretRef 显式配置但无法解析，解析失败为默认拒绝（不会被 remote 回退掩盖）。
- `trustedProxies`：终止 TLS 的反向代理 IP。仅列出你控制的代理。
- `allowRealIpFallback`：为 `true` 时，当缺失 `X-Forwarded-For` 时，网关接受 `X-Real-IP`。默认 `false`，实现默认拒绝。
- `gateway.tools.deny`：对 HTTP `POST /tools/invoke` 额外拒绝的工具名（扩展默认拒绝列表）。
...
- `POST /hooks/<name>` → 由 `hooks.mappings` 处理解析

<Accordion title="映射详情">

- `match.path` 匹配 `/hooks` 后的子路径（如 `/hooks/gmail` → `gmail`）。
- `match.source` 匹配通用路径中某个负载字段。
- 模板如 `{{messages[0].subject}}` 从负载中读取。
- `transform` 指向返回 hook 操作的 JS/TS 模块。
  - `transform.module` 必须是相对路径，且始终位于 `hooks.transformsDir` 内（绝对路径和路径穿越会被拒绝）。
- `agentId` 路由到指定智能体；未知 ID 会回退默认。
- `allowedAgentIds`：限制显式路由（`*` 或省略允许全部，空数组 `[]` 全部拒绝）。
- `defaultSessionKey`：可选固定会话键，用于未显式提供 `sessionKey` 的 hook 智能体运行。
- `allowRequestSessionKey`：允许 `/hooks/agent` 调用者设置 `sessionKey`（默认 `false`）。
- `allowedSessionKeyPrefixes`：针对显式 `sessionKey`（请求+映射）的可选前缀白名单，例如 `["hook:"]`。
- `deliver: true` 会将最终回复发送到某个渠道，`channel` 默认为 `last`。
- `model` 覆盖此次 hook 运行的 LLM（若设置了模型目录，必须是允许的模型）。
...
- 配置后，Gateway 网关启动时会自动启动 `gog gmail watch serve`。设置 `OPENCLAW_SKIP_GMAIL_WATCHER=1` 可禁用。
...
- 在 Gateway 网关端口下，通过 HTTP 提供可由智能体编辑的 HTML/CSS/JS 以及 A2UI：
  - `http://<gateway-host>:<gateway.port>/__openclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__openclaw__/a2ui/`
- 仅限本地访问：保持 `gateway.bind: "loopback"`（默认）。
- 非 loopback 绑定时，canvas 路由需要 Gateway 网关认证（token/password/trusted-proxy），与其他 Gateway 网关 HTTP 界面一致。
- Node WebView 通常不会发送认证头；配对且连接的节点，Gateway 网关会为 canvas/A2UI 访问通告带节点作用域的 capability URL。
- Capability URL 绑定到活动节点 WS 会话，并且快速过期。不使用基于 IP 的回退。
- 会向所服务的 HTML 注入 live-reload 客户端。
...
- 主机名默认 `openclaw`。可以用 `OPENCLAW_MDNS_HOSTNAME` 覆盖。
...
在 `~/.openclaw/dns/` 下写入单播 DNS-SD 区域。用于跨网络设备发现，可配合 DNS 服务器（推荐 CoreDNS）+ Tailscale split DNS 使用。

设置命令：`openclaw dns setup --apply`。
