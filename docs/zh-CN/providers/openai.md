...
当 `pi-ai`
发现 `openai-codex/gpt-5.3-codex-spark` 时，OpenClaw 也会保留它。请将其视为依赖授予且处于实验阶段：Codex Spark 与 GPT-5.4 `/fast` 分开，是否可用取决于已登录的 Codex /
ChatGPT 账户。
...
- [实时 API 和 WebSocket](https://platform.openai.com/docs/guides/realtime-websocket)
...
- 当负载未明确指定推理时，设置 `reasoning.effort = "low"`
- 当负载未明确指定冗余度时，设置 `text.verbosity = "low"`
