...
read_when:
  ...
  - 设置一台新机器
sidebarTitle: "入门：CLI"
summary: CLI 新手引导：用于 Gateway 网关、工作区、渠道和技能的引导式设置
title: CLI 新手引导
x-i18n:
...
它可在一次引导式流程中配置本地 Gateway 网关或远程 Gateway 网关连接，以及渠道、技能
和工作区默认值。
...
<Tabs>
  ...
  <Tab title="高级模式（完全控制）">
    - 暴露每一个步骤（模式、工作区、Gateway 网关、渠道、守护进程、技能）。
  </Tab>
</Tabs>
...
1. **模型/认证** —— 选择任意受支持的提供商/认证流程（API 密钥、OAuth 或 setup-token），包括自定义提供商
   ...
   安全说明：如果这个智能体将运行工具或处理 webhook/hooks 内容，请优先选择当前可用的最强最新一代模型，并保持严格的工具策略。较弱/较旧的层级更容易被提示注入。
...
- 新手引导概览：[入门概览](/start/onboarding-overview)
