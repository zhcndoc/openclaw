...
支持多账户：使用 `channels.signal.accounts`，为每个账户单独配置，并可选设置 `name`。共享模式请参见 [`gateway/configuration`](/gateway/config-channels#multi-account-all-channels)。
...
排查流程见：[故障排查](/channels/troubleshooting)。
