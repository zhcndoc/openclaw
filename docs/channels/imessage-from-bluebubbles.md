...
| `channels.bluebubbles.password`                            | _(已移除)_                               | 无需 webhook 认证。                                                                                                                                                                                                                                                                                                                                                    |
| _(隐式)_                                               | `channels.imessage.cliPath`               | `imsg` 的路径（默认 `imsg`）；如果通过 SSH，请使用包装脚本。                                                                                                                                                                                                                                                                                                                       |
| _(隐式)_                                               | `channels.imessage.dbPath`                | 可选的 Messages.app `chat.db` 覆盖；如果省略则自动检测。                                                                                                                                                                                                                                                                                                                |
| _(隐式)_                                               | `channels.imessage.remoteHost`            | `host` 或 `user@host`——仅当 `cliPath` 是 SSH 包装器且你希望通过 SCP 获取附件时才需要。                                                                                                                                                                                                                                                                            |
| `channels.bluebubbles.dmPolicy`                            | `channels.imessage.dmPolicy`              | 相同取值（`pairing` / `allowlist` / `open` / `disabled`）。                                                                                                                                                                                                                                                                                                                         |
| `channels.bluebubbles.allowFrom`                           | `channels.imessage.allowFrom`             | 配对批准按句柄继承，不按令牌继承。                                                                                                                                                                                                                                                                                                                                |
...
| `channels.bluebubbles.groups`                              | `channels.imessage.groups`                | **原样复制，包括任何 `groups: { "*": { ... } }` 通配符条目。** 每个群组的 `requireMention`、`tools`、`toolsBySender` 会一并沿用。若 `groupPolicy: "allowlist"`，空的或缺失的 `groups` 块会静默丢弃所有群组消息——见下文“群组注册陷阱”。                                                                                       |
| `channels.bluebubbles.sendReadReceipts`                    | `channels.imessage.sendReadReceipts`      | 默认 `true`。在捆绑插件下，仅当私有 API 探测可用时才会触发。                                                                                                                                                                                                                                                                                            |
| `channels.bluebubbles.includeAttachments`                  | `channels.imessage.includeAttachments`    | 结构相同，**同样默认关闭**。如果你在 BlueBubbles 上启用了附件流转，那么必须在 iMessage 块中显式重新设置这一项——它不会自动继承，否则入站照片/媒体会被静默丢弃，而且不会出现 `Inbound message` 日志行。                                                                                                     |
...
| `channels.bluebubbles.mediaMaxMb`                          | `channels.imessage.mediaMaxMb`            | iMessage 默认 16 MB（BlueBubbles 默认是 8 MB）。如果你想保留较低限制，请显式设置。                                                                                                                                                                                                                                                                          |
| `channels.bluebubbles.textChunkLimit`                      | `channels.imessage.textChunkLimit`        | 两者默认都是 4000。                                                                                                                                                                                                                                                                                                                                                                |
| `channels.bluebubbles.coalesceSameSenderDms`               | `channels.imessage.coalesceSameSenderDms` | 相同的可选项。仅适用于 DM——群聊在两个通道上都保持每条消息即时发送。启用时，如果没有显式的 `messages.inbound.byChannel.imessage` 或全局 `messages.inbound.debounceMs`，会将默认入站去抖扩大到 7000 ms。参见 [iMessage 文档 § 合并来自同一发送者的分拆 DM](/channels/imessage#coalescing-split-send-dms-command--url-in-one-composition)。 |
| `channels.bluebubbles.enrichGroupParticipantsFromContacts` | _(N/A)_                                   | iMessage 已经从 `chat.db` 读取发送者显示名称。                                                                                                                                                                                                                                                                                                                          |
...
| Threaded reply (`reply_to_guid`)                    | ✅                                  | ✅ (修复 [#51892](https://github.com/openclaw/openclaw/issues/51892))       |
| Tapback (`react`)                                   | ✅                                  | ✅                                                                            |
| Edit / unsend (macOS 13+ recipients)                | ✅                                  | ✅                                                                            |
| Send with screen effect                             | ✅                                  | ✅ (修复 [#9394](https://github.com/openclaw/openclaw/issues/9394) 的部分内容) |
...
| Read receipts and typing indicator                  | ✅                                  | ✅（取决于私有 API 探测）                                               |
| Same-sender DM coalescing                           | ✅                                  | ✅（仅限 DM；可通过 `channels.imessage.coalesceSameSenderDms` 选择启用）            |
| Inbound recovery after a restart                    | ✅ (webhook replay + history fetch) | ✅（自动：通过 since_rowid 重放遗漏消息并去重；本地模式窗口更大） |

iMessage 会恢复网关停机期间遗漏的消息：启动时，它会通过 `imsg watch.subscribe` 的 `since_rowid` 从最后分发的 rowid 重新播放，并按 GUID 去重；同时，过期 backlog 年龄边界会抑制 Push-flush 的“backlog bomb”。这通过 `imsg` RPC 连接运行，因此也适用于远程 SSH `cliPath` 部署；本地部署可以读取 `chat.db`，因此恢复窗口更大。参见 [桥接或网关重启后的入站恢复](/channels/imessage#inbound-recovery-after-a-bridge-or-gateway-restart)。
