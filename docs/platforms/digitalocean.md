...
- 通过 Serve 将网关限制为仅回环访问，并通过 Tailscale 身份头对 Control UI/WebSocket 流量进行身份验证（无令牌认证假定网关主机可信；HTTP API 不使用这些 Tailscale 头，而是遵循网关的正常 HTTP 认证模式）。
- 若要改为要求显式共享密钥凭据，请设置 `gateway.auth.allowTailscale: false`，并使用 `gateway.auth.mode: "token"` 或 `"password"`。
...
- `~/.openclaw/` — `openclaw.json`、每个 agent 的 `auth-profiles.json`、通道/提供商状态以及会话数据
