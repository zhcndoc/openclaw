...
OpenClaw 可以作为 **PeekabooBridge** 的本地、权限感知 UI 自动化代理主机。这样 `peekaboo` CLI 就可以驱动 UI 自动化，同时复用 macOS 应用的 TCC 权限。
...
- 如果未找到主机，请打开任一主机应用（Peekaboo.app 或 OpenClaw.app），并确认权限已授予。
