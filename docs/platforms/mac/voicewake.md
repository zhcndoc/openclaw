...
- **唤醒词模式**（默认）：一个始终开启的语音识别器会等待触发词（`swabbleTriggerWords`）。匹配后，它会开始捕获，显示带有部分文本的浮层，并在静音后自动发送。
- **按住说话（按住右 Option 键）**：按住右 Option 键即可立即开始捕获，无需触发词。按住时会显示浮层；松开后会完成识别，并在短暂延迟后转发，这样你可以编辑文本。
...
- **Hold Right Option to talk**：启用按住说话监视器。
- 语言和麦克风选择器、实时电平表、触发词表，以及测试器（仅本地，不会转发）。
- 如果设备断开连接，麦克风选择器会保留上次选择，显示断开提示，并在其恢复前暂时回退到系统默认。
- **Sounds**：在检测到触发词和发送时播放提示音，默认使用 macOS 的“Glass”系统声音。每个事件都可以选择任意 `NSSound` 可加载的文件（例如 MP3/WAV/AIFF），或者选择 **No Sound**。
