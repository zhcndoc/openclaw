...
这两种方式都会写入 `~/.openclaw/openclaw.json`，将引导流程标记为已完成，并让应用在下次启动时使用所选传输方式。`--local-port`/`--remote-port` 默认值为 `18789`。其他标志：`--password`、`--identity <path>`、`--ssh-host-key-policy <strict|openssh>`、`--project-root <path>`、`--cli-path <path>`、`--json`。运行 `openclaw-mac configure-remote --help` 获取完整参考。
...
- 远程主机需要与本地相同的 TCC 授权（Automation、Accessibility、Screen Recording、Microphone、Speech Recognition、Notifications）。请在该机器上运行一次 onboarding 以授予这些权限。
...
- 默认情况下，SSH 隧道要求主机密钥已被信任。请先信任该主机密钥（将其添加到已配置的 known-hosts 文件中），或者为你接受其 OpenSSH 信任策略的受管别名显式设置 `gateway.remote.sshHostKeyPolicy: "openssh"`。
- 如果将 Gateway 绑定到非 loopback 接口，请使用有效的 Gateway 身份验证：令牌、密码，或使用 `gateway.auth.mode: "trusted-proxy"` 的身份感知反向代理。
...
| Health probe failed                              | 检查 SSH 可达性、PATH，以及 Baileys（WhatsApp）是否已登录（`openclaw status --json`）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Web Chat stuck                                   | 确认网关正在远程主机上运行，并且转发端口与网关 WS 端口一致；UI 需要一个健康的 WS 连接。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Node IP shows `127.0.0.1`                        | 使用 SSH 隧道时这是预期行为。如果你希望网关看到真实的客户端 IP，请将 **Transport** 切换为 **Direct (ws/wss)**。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Dashboard works but Mac capabilities are offline | 操作/控制连接是健康的，但 companion 节点连接未连接或缺少其命令接口。打开菜单栏设备部分，检查 Mac 是否为 `paired · disconnected`。对于 `wss://*.ts.net` 的 Tailscale Serve 端点，应用会在证书轮换后检测到过期的旧 TLS leaf pins，在 macOS 信任新证书后清除过期 pin，并自动重试。如果证书不受系统信任，或主机不是 Tailscale Serve 名称，请将 `gateway.remote.tlsFingerprint` 设置为预期的证书指纹，检查证书，或切换到 **Remote over SSH**。 |
