...
- Raspberry Pi 4 或 5（建议 2GB 及以上内存）
- MicroSD 卡（16GB 及以上）或 USB SSD（性能更好）
- 电源（推荐官方 Pi 电源）
- 网络连接（以太网或 WiFi）
- 约 30 分钟
...
如果 UI 要求共享密钥认证，请将配置的 token 或密码
...
如果这是无头 Pi，请启用 lingering 一次，这样用户服务在
注销后仍能保持运行：
...
**务必使用 64 位操作系统。** Node.js 和多数现代工具需 64 位。验证命令：
