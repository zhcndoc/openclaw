...
在同一个 Control UI 标签页中，暂存的标注卡片会随其窗格和会话保留，即使发生普通路由切换和窄屏分栏窗格重新挂载。切换会话时，卡片仍限定在原始会话中，并且只有在重新访问该会话时才会恢复。硬刷新、关闭窗格、Gateway 所有者变更或应用程序关闭都会丢弃这些卡片；普通附件仍沿用其现有生命周期。
...
<AccordionGroup>
  <Accordion title="发送与历史记录语义">
    - `chat.send` 是**非阻塞的**：它会立即通过 `{ runId, status: "started" }` 返回确认，响应则通过 `chat` 事件流式传输。受信任的 Control UI 客户端还可能接收可选的确认时序元数据，用于本地诊断。
    - 聊天上传支持图像和非视频文件。图像保留原生图像路径；其他文件会作为托管媒体存储，并在历史记录中显示为附件链接。
    - 使用相同的 `idempotencyKey` 重新发送时，运行期间返回 `{ status: "in_flight" }`，完成后返回 `{ status: "ok" }`。
    - `chat.history` 响应受到大小限制，以确保 UI 安全。当转录条目过大时，Gateway 可能会截断较长的文本字段、省略占用空间较大的元数据块，并将超大消息替换为占位符（`[chat.history omitted: message too large]`）。
    - 当可见的助手消息在 `chat.history` 中被截断时，**显示更多**会通过 `chat.message.get`，使用 `sessionKey`、必要时使用当前的 `agentId` 以及转录中的 `messageId`，在行内获取完整的显示规范化转录条目。**显示更少**会恢复预览，但不会丢弃已获取的内容。如果 Gateway 无法返回更多内容，消息会显示明确的、可重试的错误，而不是静默地重复截断后的预览。
    - 助手生成的图像会作为托管媒体引用持久化。新客户端通过经过身份验证的 `artifacts.download` 解析其稳定的工件 ID，并接收短期有效且对应精确资源的媒体 URL，因此重新加载不依赖原始 base64 负载或图像 URL 中可重复使用的凭据。
    - 渲染 `chat.history` 时，Control UI 会从可见的助手文本中移除仅用于显示的内联指令标签（例如 `[[reply_to_*]]` 和 `[[audio_as_voice]]`）、纯文本工具调用 XML 负载（包括 `<tool_call>...</tool_call>`、`<function_call>...</function_call>`、`<tool_calls>...</tool_calls>`、`<function_calls>...</function_calls>` 以及被截断的工具调用块），以及泄露出的 ASCII/全角模型控制令牌。对于可见文本整体仅为精确静默令牌 `NO_REPLY` / `no_reply` 或心跳确认令牌 `HEARTBEAT_OK` 的助手条目，系统会将其省略。
    - 在正在发送以及最终刷新历史记录期间，如果 `chat.history` 短暂返回较旧的快照，聊天视图会继续显示本地乐观用户/助手消息；Gateway 历史记录追上后，规范转录会替换这些本地消息。
    - 实时 `chat` 事件表示传递状态，而 `chat.history` 则从持久化会话转录中重建。在工具最终事件之后，Control UI 会重新加载历史记录，并仅合并一小段乐观尾部；转录边界记录在 [WebChat](/web/webchat) 中。
    - `chat.inject` 会向会话转录追加一条助手备注，并广播一个用于 UI 更新的 `chat` 事件（不会运行代理，也不会向频道发送）。
    - 侧边栏会按代理分区以及固定/频道/工作/自定义/聊天分类，列出每个已加载的活动会话，并提供一个打开草稿对话框的“新建会话”操作。打开可见行只会移动高亮显示。可以将会话拖到“已固定”中以固定会话，也可以拖到自定义分组或“聊天”中移动会话；自定义分组可以折叠并通过拖拽重新排序，分组名称和顺序会通过网关同步，而折叠状态保留在浏览器中。新建的仪表板会话会根据其第一条非命令消息异步生成简洁标题；显式名称和经过身份验证的发送者身份彼此独立，因此不会将账户名称用作生成标题。当“新建会话”在未指定工作树名称的情况下创建工作树时，OpenClaw 还会使用会话标签或生成的标题作为分支名称；如果没有这些信息，则回退到一个易读的甲壳类动物主题名称。设置 `agents.defaults.utilityModel`（或 `agents.entries.*.utilityModel`）可将这一独立的模型调用路由到成本更低的模型；如果该独立模型失败，标题生成会使用主模型重试一次。展开其他代理分区后，可以浏览该代理的会话，而不会离开当前打开的聊天。
    - 线程搜索位于命令面板中（⌘K，或左上角控制区域中的搜索按钮）：输入查询后，系统会跨代理遍历有限数量的匹配页面，过滤内部子会话/定时任务行，并将可见匹配项列在导航命令旁边。“线程”页面则保留带筛选器的完整可搜索列表。
    - 每个侧边栏行都保留直接固定访问入口，以及包含未读状态、重命名、分叉、分组、归档和删除的完整上下文菜单。多选行（Cmd/Ctrl-单击，Shift-单击选择范围）会获得包含未读状态、分组、归档和删除的批量菜单；批量归档会在归档符合条件的行时报告每个会话的失败情况，而批量删除则保留其独立的“空闲或已归档”资格要求。代理主会话（包括全局作用域中的 `global`）和 `unknown` 哨兵会话始终禁用归档。对于其他任何会话，包括有活动工作的会话，Gateway 会先停止并彻底排空该会话的工作，然后再将其归档。选中的已归档会话会继续保持打开状态，并显示归档通知和**取消归档**操作；删除选中的会话后，Chat 会切换回该代理的主会话。
    - 在 macOS 应用中，OpenClaw 标志会使用窗口控件旁边原本空置的原生标题栏区域，而不会占用一个侧边栏行。
    - 在桌面宽度下，聊天控件会保持在一行紧凑排列，并在向下滚动转录内容时折叠；向上滚动、返回顶部或到达底部时，控件会恢复。
    - 当其他人正在查看同一会话时，会话标题栏会在工作区标签旁显示一个小型头像堆叠；它最多列出四个查看者头像，并显示超出数量；当只有你一人时会消失。
    - 连续的重复纯文本消息会合并为一个气泡，并显示数量徽章。包含图像、附件、工具输出或画布预览的消息不会折叠。
    - 用户消息气泡包含转录操作：悬停时显示的回退按钮（带有“不要再询问”选项的确认弹出框），以及右键菜单中的**回退到此处**和**从此处分叉**。回退会将会话指向该消息之前的状态，并将其文本返回编辑器，以便编辑和重新发送（`sessions.rewind`、`operator.admin`）；分叉会从该消息之前的活动路径前缀创建新会话，打开新会话，并用相同文本填充其编辑器（`sessions.fork`、`operator.write`）。代理工作期间，这两个操作都会禁用并显示解释性工具提示；它们仅适用于已持久化的用户消息，并且对于对话由外部代理运行框架拥有的会话会被拒绝。回退只会移动聊天上下文——文件和其他工具副作用不会还原——而回退前的转录仍会保留在追加式会话存储中。当该存储包含多个转录分支时，聊天标题栏会显示分支菜单，其中包含每个分支的最新消息、消息数量和最近活动时间；选择非活动分支会将当前会话切换回该保留路径（`sessions.branches.list`、`operator.read`；`sessions.branches.switch`、`operator.admin`）。代理工作期间同样无法切换分支，选择已处于活动状态的分支会在 RPC 边界返回一个带类型的无操作错误。
    - 当会话的检出位于 GitHub 仓库的非默认分支上时，聊天视图会在编辑器上方固定显示拉取请求标签：PR 编号、仓库、分支、差异统计、CI 标签以及草稿/已合并/已关闭状态，每项都链接到对应 PR。该行最多显示两个标签——实时的（开放/草稿）PR 优先——“显示更多”按钮会展开折叠的已合并/已关闭历史记录。CI 标签会打开一个小型 CI 监控弹出框，其中显示通过/失败/运行中/已跳过的检查数量，以及指向 PR 检查页面的链接。Gateway 只会轮询已连接 Control UI 中可见的会话，并通过 `controlUi.sessionPullRequests.changed` 推送变化后的快照；如果已设置 `GH_TOKEN`/`GITHUB_TOKEN`，则会复用它们。当达到 GitHub API 速率限制时，标签会保留已知的最新状态，并显示状态可能已过时的警告；关闭标签后，它会在当前浏览器配置文件中对该会话隐藏。在存在 PR 之前，该行会显示分支本身——仓库、分支名称，以及相对于默认分支合并基准的差异大小（已提交和未提交的工作）。推送的分支一旦有可供比较的提交，该行就会添加“创建 PR”按钮，打开 GitHub 的新建拉取请求页面；在此之前，即使会话包含已更改文件（已提交、未提交或未跟踪），仍会显示该行但不显示按钮。当存在开放或草稿 PR 时，该行会隐藏；分支的 PR 合并且推送的分支顶端仍与已合并的头提交匹配后，该行也会消失（只有在出现新的本地工作时才会重新显示且不带“创建 PR”按钮，待新提交推送到已合并头提交之后才会再次带有该按钮）。分支行仅来自本地 git，因此即使 GitHub 受到速率限制仍然可用，并带有相同的过时状态警告，因为在限制重置前无法信任“未找到 PR”这一结果。
    - 会话差异面板显示会话检出实际发生的变化：工作区轨道或聊天标题栏中的分支按钮会打开详细面板，显示相对于检出默认分支合并基准的分支、未提交和未跟踪工作的逐文件差异——状态点、重命名箭头、每个文件的 +/- 数量、可折叠文件，以及代码块之间的“N 行未修改行”标记。差异通过 `sessions.diff` Gateway 方法在服务器端计算（`operator.read` 作用域）；二进制文件和过大的文件会降级为仅统计条目，并且只有在已连接 Gateway 声明支持 `sessions.diff` 时才会显示该按钮。
    - 每个 Chat 窗格都有标题栏。点击会话标题即可重命名；工作区标签会复制检出路径或分支，并可在主机文件管理器中显示本地 Gateway 工作区。远程会话和执行节点会话保留复制操作，但隐藏显示位置操作。
    - 每个 Chat 窗格中的线程工作区轨道会列出线程文件、项目文件和工件。默认停靠在窗格右边缘；拖动其标题（或使用停靠按钮）可将其移到底部，选择会存储在当前浏览器配置文件中。折叠后的轨道完全不占用空间：使用 ⇧⌘B 或标题栏中的文件切换按钮重新打开，文件切换按钮会显示已更改文件数量徽章。独立的文件、工具和 Canvas 详细面板不受影响。
    - 点击聊天中的文件引用、展开的读取/编辑/写入工具卡片中的文件路径，或工作区轨道中的文件行，会打开文件详细面板。UTF-8 文本文件使用基于 CodeMirror 的代码视图，并提供语法高亮、行号、跳转到行、文件内搜索、复制操作和“在外部编辑器中打开”菜单。不超过 256 KiB 的 AVIF、GIF、JPEG、PNG 和 WebP 图像会直接渲染；其他二进制文件会显示元数据，不进行有损文本解码。当 Gateway 向 `operator.admin` 连接声明支持 `sessions.files.set` 时，文本面板会增加编辑模式，并支持脏状态跟踪和 Cmd/Ctrl-S 保存；未保存的草稿会在当前浏览器标签页内跨文件、面板和会话导航保留，直到明确保存或丢弃。保存操作会根据 `sessions.files.get` 返回的内容哈希执行比较并交换：如果文件自加载后已在磁盘上发生变化（例如代理仍在继续工作），面板会显示冲突通知，并提供“重新加载”（采用最新内容）和“覆盖”（保留本地编辑）操作。写入操作会使用与读取相同的文件系统安全工作区保护——路径包含检查、符号链接/硬链接拒绝以及 256 KiB UTF-8 上限——并且只会覆盖现有文件；编辑器不会创建或删除文件。
    - 每个 Chat 窗格中的后台任务轨道会列出当前代理的后台任务和子代理（按代理作用域调用 `tasks.list`，并由 `task` 事件保持实时）：运行中的工作会显示实时经过时间计时器、工具使用次数、当前使用的工具和停止控制；可折叠的已完成部分还会显示运行时长。选择某一行后，列表会在同一轨道中替换为紧凑的详细视图；返回按钮会返回列表，检查子代理不会用子转录替换主对话。使用标题栏中的活动切换按钮打开该轨道；任务快照会预先加载，因此即使不打开轨道，也会显示运行数量徽章。“任务”页面仍是跨代理的完整记录。
    - 工作区轨道、后台任务轨道和详细面板会根据各自窗格的宽度而不是窗口宽度进行适配：在窄窗格或紧凑窗口中，两个轨道都会显示为底部条带（窗格变宽前隐藏侧边停靠控件；只有一列可容纳时，工作区轨道优先占用侧边位置），详细面板会堆叠在线程下方，并使用水平调整大小控件，而不是与线程共享同一行。手机大小的视口仍会以全屏方式打开详细面板。
    - 聊天标题栏中的模型和思考选择器会通过 `sessions.patch` 立即修改当前会话；它们是持久化的会话覆盖项，而不是仅针对单次发送的选项。
    - **拆分视图：**从聊天标题栏打开（位于线程差异、后台任务和线程文件切换按钮旁边），然后将活动窗格向右或向下拆分，数量不限，直到空间容纳不下为止。每个窗格都有自己的线程、转录、编辑器和工具流。
    - 具有 `screen` 工具的代理可以在有能力的 Control UI 连接时请求相同的窗格、侧边栏、终端、浏览器、焦点和导航更改。协议 v1 会将命令应用于每个已连接且具备能力的 Control UI；请参阅 [Screen](/tools/screen)。
    - 将侧边栏中的会话拖入聊天即可在窗格中打开它。动画拖放预览会在各区域之间移动，并标记结果——新窗格将占用的确切半区上方显示“拆分”，整个窗格上方显示“在此打开”；单窗格模式同样支持拖放。
    - 活动拆分窗格会驱动侧边栏选择和 URL。其标题栏会增加拆分和关闭控件；分隔线可以调整列和堆叠窗格的大小，浏览器会在重新加载后将布局保存在本地。
    - 在窄屏幕上，拆分视图会保留布局，但只渲染活动窗格，包括带关闭控件的标题栏。
    - 如果在同一会话的模型选择器更改仍在保存时发送消息，编辑器会等待该会话补丁完成后再调用 `chat.send`，以确保发送使用所选模型。
    - 输入 `/new` 会创建并切换到与“新建聊天”相同的新仪表板会话，但当配置了 `session.dmScope: "main"` 且当前父会话是代理的主会话时除外；此时它会原地重置主会话。输入 `/reset` 会保留 Gateway 针对当前会话的显式原地重置行为。
    - 聊天模型选择器会请求 Gateway 配置的模型视图。如果 `agents.defaults.modelPolicy.allow` 非空，则该策略会驱动选择器，包括使提供商作用域目录保持动态的 `provider/*` 条目。否则，选择器会显示已配置的条目以及具有可用身份验证的提供商；`agents.defaults.models` 下的别名和设置不会限制它。完整目录仍可通过调试 `models.list` RPC 并使用 `view: "all"` 获取。
    - 当新的 Gateway 会话使用量报告包含当前上下文令牌时，聊天编辑器工具栏会显示一个小型上下文使用环，并标出已使用百分比。打开该环可查看当前上下文窗口、最近一次运行的令牌数量和估算总成本、提供商/模型身份，以及在有报告时显示的最新提供商响应输入/输出/缓存成本明细。在上下文压力较高时，该环会切换为警告样式；达到建议的压缩级别后，还会显示一个紧凑按钮，用于运行正常的会话压缩流程。在 Gateway 再次报告新使用量之前，过时的令牌快照会被隐藏。
