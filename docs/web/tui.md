...
- 在配置文件已经写入设置后，`openclaw` 和 `openclaw crestodian` 也会使用这个 TUI shell，其中 Crestodian 作为本地设置与修复的聊天后端。
...
- 会话选择器：显示当前 agent 在最近 7 天内更新的最多 50 个会话。使用 `/session <key>` 跳转到较早的已知会话。
- 设置：切换交付、工具输出展开和思考内容可见性。
