...
- 只有在可用 API 密钥的情况下才会尝试 Firecrawl 回退（`tools.web.fetch.firecrawl.apiKey` 或环境变量 `FIRECRAWL_API_KEY`）。
- `maxAgeMs` 控制缓存结果允许的最大年龄（毫秒），默认是 2 天。

## 隐身模式 / 反机器人

Firecrawl 提供了一个 **代理模式** 参数用于反机器人（选项为 `basic`、`stealth` 或 `auto`）。  
OpenClaw 对 Firecrawl 请求总是使用 `proxy: "auto"` 并设置 `storeInCache: true`。  
如果省略代理参数，Firecrawl 会默认使用 `auto`。`auto` 会在基础代理失败时尝试隐身代理，这可能会比仅使用基础代理消耗更多积分。
