...
- 需要所选提供商的 API 密钥（推荐执行 `openclaw configure --section web`）。
...
- 路径中可包含以斜杠分隔的插件 ID（针对 `plugins.entries.<id>`），例如 `plugins.entries.pack/one.config`。
...
注意：当设置了 `gatewayUrl` 时，需要显式包含 `gatewayToken`。工具不会继承配置或环境凭证用于覆盖，缺少显式凭证会导致错误。
...
- 故障排查指南：
  - Linux 启动/CDP 问题：[浏览器故障排查（Linux）](/tools/browser-linux-troubleshooting)
  - WSL2 Gateway + Windows 远程 Chrome CDP：[WSL2 + Windows + 远程 Chrome CDP 故障排查](/tools/browser-wsl2-windows-remote-cdp-troubleshooting)
