...
当你需要本地文件、你的 shell、你的仓库或配对
设备时，不要使用它。请使用 [`exec`](/tools/exec)。

## 设置

你需要一个 xAI API 密钥。以下任意一种都可以：
...
该工具在内部只接受一个 `task` 参数，因此代理应将
完整的分析请求以及任何内联数据放在同一个提示中发送。

## 限制

- 这是 xAI 的远程执行，不是本地进程执行。
- 它应被视为一次性的分析，而不是持久化的笔记本。
- 不要假设可以访问本地文件或你的工作区。
- 如需最新的 X 数据，请先使用 [`x_search`](/tools/web#x_search)。
