...
| 字段                             | 默认值 | 作用                                                                                                                          |
| -------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------- |
| `enabled`                        | `false` | 滚动历史检测器的主开关。设为 `false` 也会禁用压缩后护栏。                                                                      |
| `historySize`                    | `30`    | 保留用于分析的最近工具调用次数。                                                                                              |
| `warningThreshold`               | `10`    | 模式被归类为仅警告之前的阈值。                                                                                                |
| `criticalThreshold`              | `20`    | 阻止重复无进展循环模式的阈值。                                                                                                |
| `unknownToolThreshold`           | `10`    | 在此次数的未命中后，阻止重复调用同一个不可用工具。                                                                            |
| `globalCircuitBreakerThreshold`  | `30`    | 跨所有检测器的全局无进展断路器阈值。                                                                                          |
| `detectors.genericRepeat`        | `true`  | 对重复的同工具 + 同参数模式发出警告；当相同调用还返回完全相同的结果时则阻止。                                                  |
| `detectors.knownPollNoProgress`  | `true`  | 检测无状态变化的已知轮询类模式。                                                                                              |
| `detectors.pingPong`             | `true`  | 检测交替 ping-pong 模式。                                                                                                     |
| `postCompactionGuard.windowSize` | `3`     | 压缩后护栏保持武装状态的工具调用次数，以及触发中止所需的相同三元组计数。                                                     |
