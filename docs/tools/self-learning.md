...
审查器处于隔离环境中，并倾向于捕获规模较小且证据充分的内容。它可以查看有界的工作区技能列表、列出或检查提案，并读取可写技能的有界摘录以获取上下文。它最多会起草一份待处理提案：优先修改匹配的待处理提案，其次修补现有的、负责规范该项工作的技能，只有在没有任何技能覆盖该类别时才创建新技能。修补提案会引用需要修改的确切现有文本（或追加一个新章节），工具会在对提案进行哈希绑定的同一次读取中合成完整正文，因此未触及的内容会凭机制得到保留，并且修补会在 `auto` 模式下自动应用。修补要求具备完整技能读取回执：超出有界读取预算的技能无法自主修补。完整正文更新重写始终会保持待处理状态，交由操作员审查。其单次变更预算在重试之间共享。每次变更都是一份待处理提案——它绝不会直接写入现有技能，也不能执行应用、拒绝、隔离、发送消息或使用通用代理工具。经过审查的轨迹是证据，而不是指令。
...
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `off`     | 不创建体验审查捕获。                                                                                                                                      |
| `propose` | 创建或修订待处理提案。不会自动应用任何内容。                                                                                                             |
| `auto`    | 创建或修订提案，然后通过正常的 Workshop 应用流程应用新技能和补丁提案。完整主体更新仍会保持待处理状态以供审查。这是默认模式。 |
