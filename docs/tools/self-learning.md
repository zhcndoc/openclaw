...
生成的提议遵循共享的编写标准：类级名称、
以任务或触发条件开头的一句话描述，以及简洁、基于证据的祈使步骤。
它们保留受支持的陷阱和验证检查，记录可行的修复，
且不会虚构命令、路径、标志或 API。
...
- 前台轮次必须成功完成。
- 当前轮次必须至少包含十次模型迭代。
- Cron、heartbeat、memory、overflow、hook、subagent 和 review 会话均被排除。
- 前台运行必须已解析出提供方和模型，并且确实曾经获得过 `skill_workshop` 的访问权限。
- OpenClaw 会在完成后等待 30 秒。若同一会话中稍后又有一次前台完成，则会重新开始这段静默期。
- 如果任何 agent 或 reply 运行仍处于活动状态，review 会再等待 30 秒。
- 同一时间只能运行一个经验复盘。
- 延迟复盘是进程本地的 Gateway 工作。Gateway 必须在空闲窗口期间持续运行。基于 CLI 的 harness 只有在报告了解析后的模型、精确的模型迭代次数，以及实际的 `skill_workshop` 可用性时，才能安排复盘。Codex app-server harness 会为其 `openai/*` 会话报告这些事实；缺失这些事实的运行时仍然会默认失败。

前台答案绝不会为了学习而被延迟。失败或不符合条件的轮次不会启动经验复盘，不过当自动自治被禁用时，仍然可以将直接的用户纠正作为建议提供。

## 运行时支持

延迟体验审查要求运行时报告其已解析的模型以及
实际的 `skill_workshop` 可用性。嵌入式运行器和 Codex
应用服务器宿主环境会提供这些信息；Codex 还会报告其确切的模型
迭代次数。其他基于 CLI 的运行时在提供相同的运行时信息之前都会默认失败。确定性修正捕获和 `/learn` 仍可在
这些运行时上工作。
...
对于 Codex app-server 会话，OpenClaw 将唯一的上游响应完成次数计为模型迭代，并使用 harness 固定的当前轮投影。harness 还会尝试在延迟审查安排之前，将该投影镜像到代理的 SQLite 转录中；审查者无需重新读取数据库。

审查者会复用已解析的提供方和模型。当该身份可用时，它会复用前台认证配置文件，并禁用模型回退。因此，审查会在已配置的提供方上开始一次额外的模型运行。该运行在检查或起草提案时可能会发出不止一次提供方请求。提供方定价和数据处理条款与前台轮次同样适用。
...
只有应用操作会写入一个 فعال的 `SKILL.md`。有关完整的生命周期和存储
