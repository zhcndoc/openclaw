...
npm 规范仅支持 **注册表包名 + 可选版本/标签**。不支持 Git/URL/文件规范。
...
- Microsoft Teams 2026.1.15 版起仅提供插件形式；若使用 Teams，请安装 `@openclaw/msteams`。
...
开发插件时，请使用 SDK 子路径替代巨大 `openclaw/plugin-sdk` 导入：
...
若多个插件声明 `kind: "memory"`，只有选中的加载，其他禁用并生成诊断。
...
- `acceptsArgs`：是否接受参数（默认 false），假如为 false 但带参数，消息不会匹配，交由其他处理
